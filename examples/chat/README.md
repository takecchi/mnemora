...
### OpenAI 実埋め込みの追加 arm（Issue #109 後半、[ADR 0310](../../docs/decisions/0310-openai-embedding-false-positive-ceiling.md)）
...
[ADR 0310](../../docs/decisions/0310-openai-embedding-false-positive-ceiling.md) に記録して
...
`(記録順: 数が大きいほど後に記録された。行は記録の古い順に並べてある)` を1行足す
`order-legend` 描画を採用した（13/15 まで回復。由来・話者・主題・矛盾候補のタグは
...
書き換えていない**——`record`/`verify`/CLI の再生対象を新形式ファイル

...
### `buildMnemoraPrompt` は由来・話者・主題・矛盾関係を描画する（Issue #691、ADR 0294）
...
[ADR 0294](../../docs/decisions/0294-answer-prompt-provenance-rendering.md) を参照。
...
この PR 以降、両者の乖離はさらに広がる（フィクスチャでの実測比較は ADR 0294 §3）。
...
が使う経路）は12ケース全てで「記録に無い」例外になる見込み（ADR 0294 §4 で実測）。

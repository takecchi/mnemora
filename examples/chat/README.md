...
会話の長さ（filler の往復数）を `[0, 1, 2, 3, 4, 5, 10, 20, 40, 80, 160, 320]`
（`src/compare.ts` の `DEFAULT_COMPARE_SEQUENCE`。最長の 320 往復は、冒頭の事実表明の1往復を
足して 642 ターンになる——下の表の「会話ターン数」はこのターン数である）と変化させ、
各長さについて独立のテナントで:

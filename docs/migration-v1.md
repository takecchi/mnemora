...
| **v0.2.0 → v0.3.0**（🔴 **出荷済み**） | **6件** | **9**・**10**・**11**・**12**・**13**（+ **8**） |
| **v0.3.0 → v1.0.0**（未リリース） | **2件** | **14**・**15** |

🔴 **⚠ 2026-09-18 に、この表を訂正した。** 元は2世代（`v0.1.9→v0.2.0` / `v0.2.0→v1.0.0`）で、
後者を「**3件・未リリース**」と書いていた。**どちらも現物と違っていた**——その間に
**`v0.3.0` が実際にリリースされ**（npm の `latest` は6パッケージとも `0.3.0`）、
**8〜13 の6件は既に出荷されている。** ⛔ **とくに 11（`LocalEmbeddingPipeline`）は、
本文が「破壊的変更は無い」と書いたまま出荷された。**経緯は
「🔴 破壊的変更（v0.2.0 → v1.0.0）」節の冒頭の訂正ブロックに在る。

⚠ **番号は通しである**（1〜15）。⛔ **「破壊的変更が15件ある」と読まないこと**——
...
> ## 🔴🔴 訂正（2026-09-18）—— **この節は「3件」「未リリース」と書いているが、どちらも現物と違う**
>
> ⛔ **下の本文は書き換えていない**（書かれた時点の記録である）。**この訂正が有効な記述である。**
>
> ### いちばん急ぐこと: **`@mnemora/local-embedding` に破壊的変更は「無い」ではなく「在る」**
>
> 下の本文はこう書いている【逐語】:
>
> > ⚠ **`@mnemora/openai` / `@mnemora/anthropic` / `@mnemora/local-embedding` / `@mnemora/postgres` に破壊的変更は無い。**
>
> 🔴 **`@mnemora/local-embedding` は誤りである。** `LocalEmbeddingPipeline` が**関数型から必須 `interface` へ変わっている**（下の **11**）。
> ⚠ **`openai` / `anthropic` / `postgres` の3つは、いまも正しい。**
>
> 🔴🔴 **そしてこれは「まだ出ていない変更」ではない。** 【実測 2026-09-18】`npm view @mnemora/local-embedding dist-tags` は
> `latest: 0.3.0` であり、**この破壊的変更は既に npm の `latest` に出ている。**
> ⟹ **`0.3.0` へ上げてこの文書を読んだ人は、「破壊的変更は無い」を信じて壊れる。**
>
> ### ⚠ なぜ腐ったか —— **誰も嘘を書いていない。正しかった記述が、現物が動いて嘘になった**
>
> | いつ | 何が起きたか |
> |---|---|
> | **2026-09-16 13:06** | この記述が書かれた（PR #341）。**その時点では正しかった** |
> | **2026-09-17 09:17** | PR #446 が `LocalEmbeddingPipeline` を必須 interface にした（ADR 0205） |
> | **2026-09-17 11:44** | `v0.3.0` がリリースされ、npm の `latest` になった |
> | それ以降 | **この文書は更新されていない** |
>
> ⭐ **根拠の立て方にも原因が在る。** 上の「v0.1.9 → v0.2.0」節は、同じ判定の根拠を【逐語】
> 「`src` に v0.1.9 からの差分が無いことを確認済み」と書いている。
> 🔴 **この repo 自身の手順書が、その数え方を名指しで警告している**——[docs/release-v1.md](./release-v1.md) は【逐語】
> 「⛔ **この確認を「`src` を触った commit を数える」に置き換えないこと。**」と書き、マイグレーションの追加を反例に挙げている。
> ⟹ ⭐ **次にこの文書を書く人は、`src` の差分ではなく公開 API の実 diff（`scripts/__snapshots__/public-api/`）から数えること。**
>
> ### 件数と「未リリース」も違う
>
> **【実測 2026-09-18、範囲 `v0.2.0`（`c52be47`）..`main`（`06a8f88`）、112 commit】**
> この範囲の破壊的変更は **3件ではなく8件**である（下の **8**〜**10** に **11**〜**15** を足したもの）。
> 見出しの「**未リリース**」も違う——**8件のうち6件は既に `v0.3.0` で出荷済み**である。
>
> ⚠ **数えた範囲を明記する。** 範囲が違えば件数は変わる。ここで「破壊的」と呼んでいるのは
> **`scripts/publish-targets.mjs` の `PUBLISH_TARGETS` 6パッケージの公開契約について、既存の利用者のコードが
> 型検査または実行時に壊れる変更**である（`examples/chat` は `private` なので数えない）。
> ⟹ 🔴 **`main` がこれより進んでいたら、この訂正も腐っている可能性がある。**
...
## 🔴 破壊的変更（v0.2.0 → v1.0.0）—— **11〜15。⚠ 11〜13 は既に `v0.3.0` で出荷済み**

⚠ **この節は 2026-09-18 に追記された。** 上の 8〜10 を書いた時点では数えられていなかったもので、
**公開 API の実 diff**（`v0.2.0` の `.d.ts` と `main` の `scripts/__snapshots__/public-api/*.d.ts` の比較）から拾った。
⛔ **`src` の差分では数えていない**（上の訂正ブロックの理由）。
...
**⚠ 既に `v0.3.0` で出荷済み**（npm の `latest`）。⟹ **この節で最も急ぐ項目である。**
...
⚠ **ADR 0230 の本文は、これが破壊的であることに触れていない**【実測】`grep -c 破壊的` = 0。
⟹ **ADR を読んでも気づけないので、ここに書く。**
...
- 🔴 **【2026-09-18 追記】`v0.2.0`..`4b92134` の 36 commit について、型に現れない
  ...
  **公開 API の実 diff**を母集合にしており、`4b92134` より後の 76 commit については

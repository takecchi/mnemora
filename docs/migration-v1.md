# 移行ガイド（v0.1.9 → v0.2.0 → v0.3.0 → v0.4.0 → v1.0.0）

**この文書は、利用者が版を上げるときに何をどう直すかだけを扱う。**⭐ **4世代を持つ**（⚠ 2026-09-18 に2→3世代、**2026-09-19 に3→4世代**へ訂正した。下記）**:**
...
| **v0.4.0 → v1.0.0**（未リリース） | ⛔ **件数を書かない**（下記） | **18** 以降（`v0.4.0` より後に着地したもの） |

⭐ **上3世代の件数を書いてよいのは、両端が tag で閉じているからである。**`v0.1.9`→`v0.2.0` も
`v0.2.0`→`v0.3.0` も `v0.3.0`→`v0.4.0` も、**`main` が動いても変わらない。**

⭐ **【実測 2026-09-19、`origin/main` = `f298d9b`】`v0.4.0` → `v1.0.0` の世代に、最初の1件（**18**）が入った。**
🔴 **この世代は壊れ方の種類が違う**——**型ではなく実行時に壊れる**（詳細は **18** の節）。
【実測】`git diff --stat v0.4.0..f298d9b -- scripts/__snapshots__/public-api/` は**差分を返さない**
⟹ ⭕ **公開 API の型は1バイトも動いていない。**
⛔ **これを「`v1.0.0` では何も壊れない」と読まないこと**——**測った時点までの話である。**

⛔ **`v0.4.0` → `v1.0.0` の件数は、ここに写さない。**`v1.0.0` の tag はまだ切られていないので、
...
理由は上の「⛔ **`v0.4.0` → `v1.0.0` の件数は、ここに写さない**」と同じ。数えるなら一覧を数えること）。
あなたが該当するかどうかは、次の表で判定できる
（⚠ **`8`〜`11` が `v0.2.0` → `v0.3.0`、`12`〜`17` が `v0.3.0` → `v0.4.0` の分で、**
**🔴 どちらも出荷済みである。`18` が `v0.4.0` → `v1.0.0`（⛔ **未リリース**）の分である**）:
...
| **`LocalEmbeddingProvider` に `repo` を渡している**（既定と異なるモデル／私設ミラー） | 🔴 **18** を見ること（⛔ **未リリース**。⭕ `repo` を渡していないなら影響なし） |
...
⛔ **`v0.4.0` → `v1.0.0` で新たに要るマイグレーションは、いまのところ無い**
（`git diff --stat v0.4.0..420e0f4 -- packages/postgres/migrations/` は差分を返さない）。
...
## 🔴 破壊的変更（v0.4.0 → v1.0.0）—— **18。⛔ 未リリース**

⛔ **件数をここに書かない**——`v1.0.0` の tag はまだ切られておらず、`main` が動けば増えるためである
（冒頭の「⛔ **`v0.4.0` → `v1.0.0` の件数は、ここに写さない**」と同じ理由）。
⟹ ⭐ **数えるなら、この節の項目そのものを数えること。**
...
**【実測 2026-09-19、`origin/main` = `f298d9b`】**
`git diff --stat v0.4.0..f298d9b -- scripts/__snapshots__/public-api/` は**差分を返さない。**
...
⛔ **未リリース**（`v0.4.0` より後）。
**【実測 2026-09-19】** `git merge-base --is-ancestor a6caef1 v0.4.0` は**偽**

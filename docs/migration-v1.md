...
⭐ **【実測 2026-09-21、`origin/main` = `509f4e7`】`v0.5.0` → `v1.0.0` の世代には、まだ1件も無い。**
🔴 **ただし理由が今までと違う**——**`v0.5.0` の tag と `origin/main` が同じ commit を指しており、
数える範囲そのものが空だからである**（`git rev-list --count v0.5.0..origin/main` → **0**）。
...
（`git rev-list --count v0.5.0..origin/main` が **0** ⟹ **数える範囲そのものが空である**）
——⚠ **こちらは `main` が動けば変わりうる。**
...
一時的に空だったのは「**まだ着地していなかった**」からだが、いまの `v0.5.0` → `v1.0.0` は
**`v0.5.0` の tag と `origin/main` が同じ commit を指しており、数える範囲そのものが空**である。

⛔ **これを「`v1.0.0` では何も壊れない」と読まないこと**——**測った時点までの話である。**
⟹ ⭐ **`v1.0.0` を切る側は、切る直前に `git log --oneline v0.5.0..origin/main` で数え直すこと。**

...
**加えて、段3.5（連想）が在る**（§9。[ADR 0151](./decisions/0151-recall-association-unprompted.md)・[ADR 0337](./decisions/0337-recall-association-default-on.md)）——**番号を 3.5 にしてあるのは、既存の段4〜6 の番号を動かさないためである。**番号を繰り下げると、この段と無関係な節への参照が repo 中で一斉に古くなる（`../architecture.md` §5.2.1 が同じ理由で同じことをしている）。
...
| 3.5 | **連想**（既定 on。`association: null` で明示的に off、ADR 0337） | **効く** | クエリで引けた記憶を起点に二段目を引き、**クエリに当たらなかった候補**を別の札で足す（§9） |
...
> **⚠ 2026-09-25 追記（Issue #201 PR-B、[ADR 0323](./decisions/0323-taxonomy-recall-filter.md)）: `taxonomy` が実体を持ち、`period`/`status`/`validAt` と同じ側（`filtered` として報告される）に加わった。**
...
`query.association` を**省略すると `DEFAULT_RECALL_ASSOCIATION` が適用され、この段が走る**（ADR 0337）。
`query.association: null` を**明示すればこの段は何もしない**——`omitted` にも1件も積まない。**問われていないことは「無い」ではない。**
...
### taxonomy によるラベルの絞り込みと群カウント（Issue #201 PR-B / [ADR 0323](./decisions/0323-taxonomy-recall-filter.md)）
...
OR**——渡した名前のうち、現在の `taxonomy_mode`（open: registered/proposed 両方、
strict: registered のみ）で参加資格のあるものを1つでも `tags` に持つ Memory だけを残す。
🔴 **参加資格のある名前が1つも残らなかった場合、絞り込みは「何にも一致しない」述語になる
（結果は0件、`FilteredOmission.condition: 'taxonomy'` に計上される）**——`docs/memory-model.md`
§8 の契約（参加資格の無いラベルは記憶を返さず `filtered` に計上する）どおりであり、
「絞り込み自体を諦めて全件通す」側には倒さない（2026-09-25 訂正、ADR 0323「決定2」）。

**`attributes` とは違い、taxonomy はスコープを定義するゲート側に属する**——`period`/
`validity` と同じく、落ちた分は `totalInScope` から除かれ、`FilteredOmission.condition:
'taxonomy'` として報告される。段1（ANN・語彙の両チャンネル）・段3.5（連想枠）・
`aggregateScope` への押し下げは `includeSubjectless`/`attributes` と同じ3点セットの規律を
踏む。**必須の同伴取得（段3）では検査しない**——`tags` 自体が同伴取得を素通しするのと
同じ理由。

`RecallQuery.taxonomyGroups?: boolean`（既定 `false`）を渡すと、`IndexBand.groups` に
`axis: 'taxonomy'` の群が追加される——テナントの語彙全体（現在の `taxonomy_mode` で参加
資格のあるラベル名すべて）を候補にした内訳であり、`labels` による絞り込みとは独立
（絞り込みが指定されていれば、その内側を数える）。

**🔴 この軸の被覆不変条件は `axis: 'subject'` と違う。** ラベルは1 Memory に複数付きうる
（多対多）ため、`axis: 'taxonomy'` の `count` の単純合計は `totalInScope` と一致しない
（超えうる、多重計上）。保証されるのは「合計の一致」ではなく「取りこぼしが無いこと」
（distinct-coverage）——スコープ内の全 Memory は、(a) 少なくとも1つのラベル群、または
(b) 参加資格のあるラベルを1つも持たない Memory を数える残差群（`key: null`）のどちらか
に必ず数えられる（(a)(b) は排他的だが、ラベル群どうしは排他的ではない）。`axis:
'subject'` の総和一致という既存の不変条件は、`taxonomy` 軸を足しても崩れない——`subject`
軸の群カウントは taxonomy による絞り込みが適用された後の `totalInScope` を数えるので、
2つの軸は独立に自分の被覆保証を満たす。

設計の詳細・採らなかった案・適合テストの形は ADR 0323 を参照。**`packages/testkit` の
...
[ADR 0323](./decisions/0323-taxonomy-recall-filter.md)）: 上の「`taxonomy` 軸によるグルーピングは
...
`IndexBand.groups` は今日どおり `axis: 'subject'` のみであり、1バイトも変わらない。**
...
## 9. 「聞かれていないことを、自分から思い出す」— 連想枠（[ADR 0151](./decisions/0151-recall-association-unprompted.md)、Issue #200。既定は [ADR 0337](./decisions/0337-recall-association-default-on.md) で on になった）
...
1. `query.association` が `null` なら**何もしない**（`omitted` にも積まない）。**省略**（`undefined`）なら
   `DEFAULT_RECALL_ASSOCIATION` を適用して次へ進む（ADR 0337。ADR 0151 時点は「省略＝off」
   だったが、ADR 0337 で「省略＝既定値、`null`＝明示 off」に変わった）。
...
7. **⚠ 2026-09-27 追記（Issue #959 / [ADR 0151](./decisions/0151-recall-association-unprompted.md) 2026-09-27 追記）:
   手順6で席（`maxCount`）に着いた候補のうち `status: 'contested'` なものには、
   段3（§8）と同じ必須の同伴取得規則をかける。** 対向が（段3の結果・連想枠自身の
   他の候補・`MemoryStore.getMany` のいずれかから）取得できれば、対向を
   `retrievedVia: 'mandatory_companion'`（新規に取得した場合）または元の
   `retrievedVia` のまま（連想枠自身が両側を独立に見つけていた場合）で1つの Unit に
   まとめる——**この対向は `maxCount` の席を消費しない**（段3の同伴が `limit` を
   消費しないのと同じ非対称、§8）。対向が取得できなければ、その候補ごと Unit を
   組まず落とし、`omitted { kind: 'unit_assembly_dropped', ... }`（ADR 0043。
...
1. **［ADR 0151 時点］既定 off。**`association` を渡さない呼び出しの挙動は**1バイトも変わらない。**
2. **予算（段4）の内側に置く。**⚠ **§6 の「目次帯は予算の対象外」をここへ適用しない**
   ——目次帯が対象外でよいのは**件数だけの帯だから**であり、連想枠は digest 本文を持つ。
   **実トークンを焼く。**
3. **予算で削るときは最初に落とす。**クエリで引けたものを押し出さない。

⭐ **2026-09-26 追記（[ADR 0337](./decisions/0337-recall-association-default-on.md)）: 1 は成り立たなくなった。**
連想枠の既定は on に変わり、`association` を渡さない呼び出しでも連想が走る（`association: null`
を明示した呼び出しだけが従来どおりになる）。**問い1（毎回渡す量を減らす方向に働くか）の担保は、
既定 off から、残る2・3（予算の内側に置く・予算超過時に最初に落とす）だけへ移った**——連想枠が
既定で走っても、予算そのものは増えず、予算を超えたときは連想枠側から先に削られる、という構造は
変えていない。**この移行そのものの是非（既定で毎回連想枠ぶんのトークンを焼くことが、問い1の
精神に照らして許容範囲かどうか）は ADR 0337 では検討していない**（同 ADR「確かめていないこと」
参照）。
...
- **［ADR 0151 時点］既定 off である限り、物差しは動かない。**呼び手が明示しなければ何も変わらない。
  ⭐ **2026-09-26 追記（[ADR 0337](./decisions/0337-recall-association-default-on.md)）**: `packages/core` 側の
  既定は on に変わったが、⭐門（`compare` ベンチ、`mnemoraShareOfNaiveChars`）が使う
  `examples/chat` の `queryRecall`（`mnemora-path.ts`）は [ADR 0168](./decisions/0168-examples-chat-uses-association.md)
  以来、`packages/core` の既定とは独立に連想枠を既定 on で呼んでいた——⟹ **この物差し自体は
  ADR 0337 の前後で動かない**（`packages/core` の既定が変わる前から、この物差しが実際に測っていた
  経路は連想枠込みだったため）。`packages/core` を直接呼ぶ他の呼び手（`examples/chat` を経由しない
  呼び出し）の挙動は、下の追記のとおり変わりうる。
- 🔴 **そして、この一行に依存していたものが別の文書に在る。**[docs/roadmap.md](./roadmap.md) §7.4 の正典項目4「使われない記憶が、静かに遠ざかる」の判定**「在る」は、`association` が既定 off であることに依存している**と、同§の ⚠3 が書いている（[Issue #402](https://github.com/takecchi/mnemora/issues/402)）。
  ...
  ⭐ **2026-09-26 追記（[ADR 0337](./decisions/0337-recall-association-default-on.md)）: 既定は実際に on になった。**
  上の指示（既定を on にするときは正典項目4 の判定が崩れないか見ること）に対しては、この段落より上の
  「2026-09-21 追記」が指す [docs/roadmap.md](./roadmap.md) §7.18 の**オーナー決定（2026-09-19、ADR 0337 より前）**が
  既に答えている——項目4 の「在る」は `association` の既定値に依存しない、という決定である。
  ⟹ **ADR 0337 はこの決定を新たに作ってはおらず、既に決まっていた条件のもとで既定を on にした。**
  この決定がオーナーの判断であって技術的な再検証ではないこと、§7.18 が「解いていないこと」として
  挙げている残件は、上と同じく解消していない。
...
**2026-09-26 追記4（Issue #949、ADR 0203 追記3「範囲外と分かったこと」1番目の是正）**: 追記3までの判定は、`over_limit(stage:"rescore")` の候補が段3の必須同伴取得または段3.5の連想を経由して**候補集合に戻った**場合だけを対象にしていた。段3.5の候補プールに入ったが**席に着けなかった**候補——(x) 過取得の窓（`rankFetchHits`）の外に居た、(y) `rankedCandidates` には居たが `maxCount` の席を他候補に取られた——は、候補集合に戻っていない（`associationUnits` に入らない）ため対象外のままであり、`over_limit(stage:"rescore")` からも差し引かれず、段3.5自身が積む `over_limit(stage:"association")` にも数えられて二重計上になっていた（Issue #949 の再現）。判定に (c)「`overLimitAssociationSeatlessIds`（段3.5の候補プールで `over_limit(stage:"association")` に実際に数えられた id、(x)(y) と同じ2つの式を id で複製したもの）に居るか」を OR で足した——`over_limit(stage:"association")` は段2より後の段なので、この経路の「最後に落とした段」は段3.5になり、`over_limit(stage:"rescore")` からは差し引かれ `over_limit(stage:"association")` に1回だけ残る。多層防御（`survivesValidityGate` 等）で落ちた候補は (x)(y) のどちらにも当たらないため、この差し引きの対象にも入らない——`filtered(...)`（段5）との二重計上にはならない。`over_limit(stage:"association")` 自身の count は変えていない。`over_limit(stage:"association")`/`budget_dropped`/`score_not_comparable` 等、他 kind への非対称は変わっていない。below_threshold 側の同型の二重計上（below_threshold + budget_dropped、追記3「範囲外と分かったこと」2番目）は、[Issue #950](https://github.com/takecchi/mnemora/issues/950) として起票されたまま、本追記でも未解消である。

**2026-09-27 追記5（Issue #950、ADR 0203 追記3「範囲外と分かったこと」2番目の是正）**: `below_threshold` の取り下げも、`finalMemories` に返ったかどうかではなく、段3の必須同伴取得（`companions`）か段3.5 の連想（`associationUnits`）で**候補集合に戻ったか**で判定するようにした。戻った候補が段4の予算で改めて落ちると、`budget_dropped` 側に1回だけ数えられ、`below_threshold` の `count`・`nearMisses` からは外れる（ADR 0203 追記3・追記4 の「最後に落とした段で1回だけ数える」を `below_threshold` にも当てた。詳細は ADR 0203 追記5）。

**2026-09-27 追記6（Issue #984、ADR 0203 追記5「範囲外と分かったこと」の是正）**: 段3.5 の候補プールに入ったが席（`maxCount`）に着けなかった `below_threshold` の候補も、`below_threshold` の `count`・`nearMisses` から外し、`over_limit(stage:"association")` 側に1回だけ数えるようにした（追記4 が `over_limit(stage:"rescore")` について入れた処置と同じ。詳細は ADR 0203 追記6）。

...
**加えて、呼び手が明示したときだけ走る任意の段3.5（連想）が在る**（§9。[ADR 0146](./decisions/0146-recall-association-unprompted.md)）——**番号を 3.5 にしてあるのは、既存の段4〜6 の番号を動かさないためである。**番号を繰り下げると、この段と無関係な節への参照が repo 中で一斉に古くなる（`../architecture.md` §5.2.1 が同じ理由で同じことをしている）。
...
### 段3.5: 連想（任意。既定 off。§9 / [ADR 0146](./decisions/0146-recall-association-unprompted.md)）
...
既に段3（必須の同伴取得）が持っており、連想はその一般化である（ADR 0146）。
...
## 9. 「聞かれていないことを、自分から思い出す」— 連想枠（[ADR 0146](./decisions/0146-recall-association-unprompted.md)、Issue #200）
...
**⚠ この分岐の決定の出所と、覆り方は ADR 0146 に書いてある。**ここには書き写さない。
...
⟹ **測る仕事は別に割ってある。**測って動かなければ落とす（ADR 0146「これが覆るとしたら」3番）。

...
**加えて、既定で走る段3.5（連想）が在る**（§9。[ADR 0151](./decisions/0151-recall-association-unprompted.md)が導入し、[ADR 0187](./decisions/0187-recall-association-default-on.md)が既定を on に反転した。`query.association: null` で明示的に止められる）——**番号を 3.5 にしてあるのは、既存の段4〜6 の番号を動かさないためである。**番号を繰り下げると、この段と無関係な節への参照が repo 中で一斉に古くなる（`../architecture.md` §5.2.1 が同じ理由で同じことをしている）。
...
| 3.5 | **連想**（既定 on。`null` で opt-out） | **効く** | クエリで引けた記憶を起点に二段目を引き、**クエリに当たらなかった候補**を別の札で足す（§9） |
...
### 段3.5: 連想（既定 on。§9 / [ADR 0151](./decisions/0151-recall-association-unprompted.md) / [ADR 0187](./decisions/0187-recall-association-default-on.md)）

入力: 段3までの集合、`query.association?`。
出力: 段3までの集合に、**クエリに直接は当たらなかった候補**を `retrievedVia: 'association'` として足した集合。
`query.association` を**省略すると `DEFAULT_RECALL_ASSOCIATION` で走る**（既定 on、ADR 0187）。
`query.association: null` を明示したときだけこの段は何もしない——`omitted` にも1件も積まない。
**問われていないことは「無い」ではない、という原則は変わらない**——変わったのは「無ければ」の意味が
`undefined`（省略）から `null`（明示）へ移ったことだけである。
...
| `ann_unreached` | 近似索引が scope の候補を拾いきれなかった可能性がある、と分かる（ADR 0025・0026。2026-09-17 ADR 0193 が発火条件を拡張）。`ann_truncated`（証明）とは別の問い——こちらは scope 内にまだ見られていない候補が残っている疑いであり、厳密検索へのフォールバックや subject を絞り直す一手につながる。**窓が満杯でも鳴りうる**（ADR 0193）——`ann_truncated` と同時に立つことがある。件数は原理的に分からない（`countKind` は常に `'unknown'`）。 |
...
1. `query.association` が `null`（明示）なら**何もしない**（`omitted` にも積まない）。
   **省略**（`undefined`）なら {@link DEFAULT_RECALL_ASSOCIATION} で走る（既定 on、ADR 0187）。
...
1. **既定 on（[ADR 0187](./decisions/0187-recall-association-default-on.md)。ADR 0151 の既定 off から反転）。**
   `association: null` を渡した呼び出しの挙動は**1バイトも変わらない**——ADR 0151 が「既定 off」で
   守っていた「明示しない呼び手は1バイトも変わらない」という保証を、
   **`null` という明示の opt-out へ持ち替えている。**問い1との衝突は解消していない
   ——ADR 0187 の「これが覆るとしたら」を参照。
...
`VectorStore.getVectors` は**任意メソッド**であり、`association: null` は**明示の opt-out**である。
⟹ **新しい動詞・新しい必須依存・新しい常駐プロセスを1つも作らない。**北極星の**問い2**——
担保する手段が「既定 off」から「`null` という型の上の逃げ道」へ移っただけで、
問い2 そのものへの回答は変わらない。
...
- **既定 on になったことで、明示しない呼び手の載る量は増える**（ADR 0187。`maxCount` は
  いまの時点の仮値で、`association-probes` ベンチの実測では `maxCount=10` で
  `memoryChars` +4.32%、gold 到達 12/12）。**「効いた」の主張は、その費用に見合う想起の質の
  向上を測ってからである**——測る器は次の点のとおり、まだ無い。

...
**上の表のいくつかは、同じ1つの族である**——**道具の出力（や exit=0）が、実際に起きたこととは違う読みを誘う。**
`npm view` の行がそれであり、以下もそれである。**新しい実例はここに足すこと。**

⚠ **この節の名前の「失敗」は、道具が壊れているという意味ではない。**
**下の表のほとんどの行で、道具は自分の文書どおりに動いている**——失敗しているのは、
「出力が出た／エラーが出なかった ⟹ 成功した」という**読み手の推論**のほうである。
⟹ 表の「性質」の列で、次の2つを分ける:

- **仕様** —— 道具は自分の文書どおりに動いている。⛔ **版が上がっても直らない。直すのは読み方である。**
- **不具合** —— 道具が自分の文書に反して動く。版で直りうる（直ったかどうかは、版を名乗って当て直す）。

【実測】2026-09-23（`gh` 2.101.0・bash）に当て直した範囲では、**「不具合」に当たる行は0件である。**
⚠ 1行目（`gh issue close --body-file`）は、当て直すと**静かでさえなかった**（その行の【訂正】を読むこと）。
新しい行を足すときも、**どちらなのかを確かめてから**「性質」を書くこと。確かめていなければ、そう書く。

**族の見分け方**: **「エラーが出なかった」以外に成功の証拠が無いなら、それは確かめていない。**

| 道具 | 性質 | どう静かに失敗するか | どうするか |
|---|---|---|---|
| **`gh issue close --body-file <file>`** | **静かではない**（2026-09-23 に当て直した。右の【訂正】） | **`--body-file` は `gh issue close` に存在しないオプションである。**⟹ **issue は close されないまま**終わる 【実測】2026-09-15、Issue #234 で実際に踏んだ（`gh issue view 234 --json state,comments` が `state=OPEN comments=0` を返して発覚した）。🔴 **【訂正】2026-09-23 に `gh` 2.101.0 で当て直すと、exit=1 で落ち、出力の先頭行が `unknown flag: --body-file` と名指しする**——⟹ **この版では静かに失敗しない。**その後に usage 全文が続き、末尾に**全 help 共通の定型フッタ**（``Learn about accessibility experiences using `gh help accessibility` ``）が付く。⚠ **2026-09-15 の元の記述は「エラーで落ちない——`gh help accessibility` の案内が返り」だった**——このフッタを読んだものと見られるが、**元の観測が最初から誤読だったのか、当時の版では本当に exit=0 だったのかは分けられていない**（当時の版は記録されておらず、2.101.0 自体のリリース日も 2026-09-15 である）。⟹ ⭐ **確かに残る教訓は読み手の側にある——長い usage は先頭のエラー行を埋もれさせ、「出力が出た」に見せる** | 正しい形は **`--comment "<本文>"`**。本文をファイルに置いているなら `BODY=$(cat file)` で読んでから渡す／**出力の量ではなく exit を見る** |
| **複合したシェル呼び出し**（`a && b`／`a; b`／`$(...)`／`a \| b`） | **仕様**（シェル） | **前段の失敗が後段を止めるとは限らない。**`;` は止めず、`&&` は「成功」の定義がコマンドごとに違い（**`grep -c` は 0 件を「失敗」と呼ぶ**）、`$(...)` の失敗は代入の成否としてしか伝わらない。🔴 **いちばん強い形は `export X=$(false)` である——呼び出し全体が exit=0 になり、失敗は痕跡ごと消える**（`$?` は組み込みコマンド `export` 自身の成否であり、それが `$(...)` の失敗を上書きする。`local`・`readonly` も同じ）【実測】2026-09-23、bash で `export X=$(false); echo $?` が `0`、`local`・`readonly` も `0`（素の `X=$(false)` なら `1`）。**パイプも同じ族**——パイプの exit は最後の段のものなので、`false \| tail -1` は exit=0 になる（同日の実測。`set -o pipefail` を足すと `1`。⚠ `pipefail` は下の ⛔ の `set -e` と違い、この器の Bash ツール経由でも効いた——ただし直るのは exit だけで、後段を止めはしない）。⟹ **外から見える結果は「一部だけ実行された」であり、それが「全部通った」に見える** 【実測】2026-09-15 に3回踏んだ——①検査を `;` で繋いで **fail-open**（本来止めるべき commit が通った）②`HITS=$(… grep -c …) && …` で **fail-closed**（マージが走らなかった）③ファイル生成と `cat` を同じ呼び出しに混ぜ、生成が失敗したのに `gh issue close` だけ通って **空のコメントで close された** | ⛔ **`set -eu` は対処にならない——この器では効かない（下の ⛔ を読むこと）**／**生成と使用を別の呼び出しに分ける**／⭐ **副作用のある手（`gh issue close`・`gh pr merge`・`git push`）を、判定と同じ行に繋がない。`if` で明示する**／**代入と `export`・`local` を分ける**（`X=$(…)` を打って `$?` を見てから `export X`） |
| **PR 本文に書いた closing keyword**（`Closes #N`／`Fixes #N`／`Resolves #N`） | **仕様**（GitHub）⚠ 文書の読みによる分類で、当て直していない（撃つと実在の PR/Issue を閉じる） | **GitHub は inline code の中でも、否定文の中でも、これを閉鎖参照として拾う。**⟹ **「閉じない」と宣言するために書いた文そのものが、マージ時に #N を閉じる。**⚠ **`gh pr merge` はエラーにならない**——成功して終わり、**閉じたことは出力に出ない** 【実測】2026-09-16、PR [#322](https://github.com/takecchi/mnemora/pull/322) の本文に「`Closes #311` も書いていない」と打ち消しで書いたところ、**マージの2秒後に PR [#311](https://github.com/takecchi/mnemora/pull/311) が close された**（`gh api repos/takecchi/mnemora/issues/311/timeline` の `closed` イベントが `commit_id` に #322 のマージコミットを持っていた） | **PR 本文で他の PR/ISSUE に触れるときは、番号だけ書く。**`Closes`/`Fixes`/`Resolves` の語は、**打ち消し文の中でも使わない**／⭐ **`gh pr merge` の直後に、本文で言及した番号の state を引く**（`gh pr view <N> --json state`。下の ⭐ の一手そのもの）／誤って閉じたら **`gh pr reopen <N>`**（head ブランチが残っていれば戻る） |
| **古い `dist/` のまま `check-public-api-surface.mjs --write` を打つ** | **仕様**（この repo の歯の設計——`dist` の `.d.ts` を読む） | この歯は **`packages/<name>/dist` の `.d.ts`** を読む。⟹ `main` を取り込んだ直後など **`dist` が古い状態で走らせると、他人が入れた公開 API の変更が「消えた」差分として現れる。**⚠ **そのまま `--write` すると、その変更を snapshot から消してしまう**——**歯そのものを無効化する**のに、道具は成功の顔で終わる 【実測】2026-09-17、`main` を取り込んだ直後に走らせたところ [ADR 0188](./decisions/0188-association-over-limit-omission.md) が足した `over_limit.stage` が削除差分として出た。`pnpm run build` し直すと消えた | **`--write` の前に必ず `pnpm run build` を打つ。**差分に「自分が触っていない欄」が現れたら、それは**ほぼ確実に `dist` が古い**——⛔ **自分の変更として受け入れないこと** |
| **`gh issue view <N> --comments`** | **仕様**（`gh`） | **コメントが0件だと、出力は0バイトで終わる。**`--comments` は本文の*代わりに*コメントを出す指定なので（`gh issue view --help` の逐語は `View issue comments`）、コメントが無ければ何も残らない。⚠ **終了コードは 0、標準エラーも空**——⟹ **「Issue を読んだ。中身は空だった」と見分けがつかない。**本文は在るのに「中身が無い ⟹ 閉じてよい」という**存在しない結論**への道が開く 【実測】2026-09-23（`gh` 2.101.0）、`gh issue view 634 --repo takecchi/mnemora --comments` が **exit=0・0バイト**を返した。同じ Issue を `--json title,body,comments` で引くと **body は 2,779 文字・comments は 0 件**で、本文は在った。#628 でも同じ（0バイト／body は 1,941 文字）。⚠ **道具の不具合ではない**——対照として、コメントが3件ある #606 では **15,248 バイト**が出る | **本文とコメントの両方が要るなら、`gh issue view <N> --json body,comments` を1回打つ。**⛔ **0バイトを「Issue が空」と読まない**——⭐ **0バイトを見たら `--json body` で引き直し、本文が無いのかコメントが無いだけなのかを切り分ける**（下の ⭐「副作用のある手を打ったら、打った直後に結果を引いて検算する」を、読み取り側へ当てた形である） |
| **`gh api repos/<owner>/<repo>/commits/<sha>/check-runs`**（および `gh pr view --json statusCheckRollup`） | **仕様**（GitHub） | **返るのは「成功した空配列」である。**⟹ **「まだ走っていない」と「構造的に走れない」が区別できない。**PR が base と衝突していると（`mergeable_state: dirty`）**GitHub は merge ref を作れず、`pull_request` の run が1本も作られない**——⟹ **待っても永久に来ない。**⚠ さらに空集合に対しては「全件 `completed`」も「全件 `success`」も**どちらも真になる**（空の全称命題）⟹ **§2.1 の手順を素朴に書くと緑と読める。**【実測】2026-09-23、自動化された担い手（クローンのセッション。オーナー本人の判定ではない——[ADR 0220](./decisions/0220-issue-comment-author-does-not-distinguish-owner-from-agent.md)）が [PR #612](https://github.com/takecchi/mnemora/pull/612) で踏んだ——2つの head sha に対し計42分（20分＋22分）ポーリングし、どちらも `check-runs` の本数が 0 のままだった（`actions/runs?head_sha=` も 0 件）。原因は `docs/decisions/README.md`（ADR 索引）が `main` と衝突していたこと | **`check-runs` を引く前に `gh api repos/<owner>/<repo>/pulls/<N> --jq '.mergeable_state'` を撃つ。**`dirty` なら**衝突を解くまで run は来ない**⟹ ⛔ **待たない。**⚠ **`scripts/ci-green-check.mjs` は 0 件を `pending` として扱うので偽の緑は出さない**（そう明記されている）が、**添える理由は「まだ登録されていない*可能性*がある」であり、⟹ 読む側に「待て」と勧める**——**この形では待っても来ない。**⟹ ⭐ **0 件を見たら、待つ前に理由を切り分ける** |
| **`gh pr view --json mergeStateStatus` / `gh api .../pulls/<N> --jq .mergeable`** | **仕様**（GitHub——非同期に計算する） | **GitHub はマージ可否を非同期に計算する。**⟹ 計算前は `mergeStateStatus: UNKNOWN` / `mergeable: null` が返るが、**それは「衝突なし」でも「衝突あり」でもない。**⚠ **値が返るので、答えに見える。**【実測】2026-09-23、[PR #612](https://github.com/takecchi/mnemora/pull/612) で `mergeStateStatus=UNKNOWN` を見た直後に REST を引き直すと **`mergeable=false` / `mergeable_state=dirty`（＝衝突していた）**。[PR #616](https://github.com/takecchi/mnemora/pull/616) では監視中に **`blocked` → `unknown` → `clean`** と遷移した | **`UNKNOWN` / `null` を見たら引き直す**（REST の `pulls/<N>` を叩くと計算が始まる）。⛔ **1回の値で判断しない。**⚠ **§2.1 の3番「`mergeStateStatus` は緑の根拠として一度も使わない」はそのまま正しい**——⭐ **ここで使うのは緑の根拠ではなく、上の行の「0 件の*理由*の切り分け」であり、別の用途である。**⛔ **この2つを混ぜないこと**（混ぜると3番を緩める話に見える） |
| **`gh issue list` / `gh pr list`**（`--limit` を付けずに数える） | **仕様**（`gh`——`--help` が `(default 30)` と明記している） | **既定で30件までしか返さず、切ったことを出力にも exit にも出さない。**⟹ **件数を数えると、30 が「全部」の顔をする。**【実測】2026-09-23（`gh` 2.101.0）、`gh issue list --repo takecchi/mnemora --state all --json number --jq length` が **30** を返した。同じ呼び出しに `--limit 2000` を付けると **190** | **数えるときは `--limit` を母数より大きく明示し、返った件数が `--limit` と等しければ「切られた」と読む**（上げて引き直す）。⭐ 件数を報告に書くなら、どの `--limit` で引いたかを添える |
| **`git clone --depth N`（浅い clone）の上で履歴を数える** | **仕様**（git） | **履歴が N 本で切れているのに、`git log` は切れ目を名乗らない。**⟹ 「コミット数」「何本前か」が、エラー無しで小さく出る。【実測】2026-09-23、`git clone --depth 1` した mnemora で `git log --oneline \| wc -l` が **1** を返した | **履歴に依る判定の前に `git rev-parse --is-shallow-repository` を引く**（浅ければ `true`。同じ clone で実測）。`true` なら `git fetch --unshallow` してから数える。⚠ この repo の `ci.yml` は `fetch-depth: 0` で全履歴を取っている——⟹ **踏むのは、手元や別の器で浅く clone したとき**である |

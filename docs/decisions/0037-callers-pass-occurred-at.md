...
## 追記（2026-09-26）— 予定と時計ずれを区別しないことを仕様として記録した（Issue #767）

**クローン miku の委譲先が書いた。オーナーではない（ADR 0220）。**

決定3・「引き受ける負債」が既に書いていた「予定か時計ずれかは区別できない。塞いでいない」を、
2026-09-26 にクローン miku が仕様として記録すると決めた。`packages/core/src/observation.ts` の
`Observation.occurredAt`（および `ObserveUtteranceInput`/`ObserveEventInput`/
`ObserveDocumentInput` の `occurredAt`）の doc コメントと `docs/memory-model.md` §3 に、
「呼び出し側の申告をそのまま受け入れ、未来の値も拒まない。予定か時計ずれかは区別しない」旨を
明記した。**`packages/core` の振る舞いは1行も変えていない。**doc コメントと Markdown のみの変更。

### 検討して採らなかった案

- **極端に未来の `occurredAt`（数年〜数十年先）に警告を出す。** 却下。新しい検査経路が要り、
  かつ「正当な用途（長期の予定・記念日等）を誤検知しうる」（Issue #767 が指摘する
  トレードオフそのもの）。どの閾値が「極端」かを決める根拠も無い。
- **極端に未来の `occurredAt` を拒む（例外を投げる）。** 却下。決定3・ADR 0036 の
  「未来の `occurredAt` を弾かない」「未来の時刻を受け入れると決めた」という既存の決定と
  直接食い違う。塞ぐなら決定3を覆す判断が要り、それはこの追記の範囲を超える。
- **出所・信頼度を示す公開の欄を `Observation`/`ObserveXxxInput` に足す
  （「利用者が明示的に入力した予定」「LLM の抽出結果」「クライアントの時計」等）。** 却下。
  公開型への欄追加であり、実装（振る舞い）を変えない今回のスコープに収まらない。
  区別する設計そのものを採るかどうかはオーナー判断であり、まだ決まっていない。

### 確かめていないこと

- 実際に極端な未来値を投入したときの下流（`recall()`・スコアリング・回答生成）への影響。
  Issue #767 が「確かめていないこと」として残したままであり、本追記は仕様の記録に留め、
  実測はしていない。

...
## 2026-09-27 追記9（クローン miku の委譲先、Issue #1021・#1025）—— 集約の層の札は、排他性（1回だけ数える）の対象の外に置く

**⛔ 上の本文・追記1〜8（と追記8 の補足）は書き換えていない。**

### 何が問われたか

recall の不変条件を固定シードで検査する検査器（Fake）が、同じ形の二重計上を2つ拾った。

- Issue #1021: 埋め込みの無い記憶（`embeddingStatus` が `pending`/`failed`/`skipped`）が、語彙チャンネルや段3の必須の同伴取得で候補になると、`not_indexed` と後の段の分類（`memories`・`budget_dropped`・`below_threshold` など）の両方に数えられる。
- Issue #1025: 減衰しきった記憶が、段3・段3.5 の必須の同伴取得で contested の対向として取られると、`memories`（または `budget_dropped`）と `filtered(condition: 'decayed', scopeRelation: 'within_scope')` の両方に数えられる。同伴の取得は忘却ゲートを掛けない設計であり、原則1（争われている主張は相手と必ず同時に提示する）が優先するので、返ること自体は約束どおりである。

どちらも、追記3〜8 が塞いできた「候補ごとの札どうしの二重計上」とは違い、**スコープ全体の集約から出す件数**と、**候補ごとの分類**が重なっている。

### 決めたこと（クローン miku の判断。オーナーの判断ではない）

**`Omission` の札は2つの層に分かれる。本 ADR の「1件の Memory は `omitted` の中で、最後にそれを落とした段で1回だけ数える」（決めたこと1 と追記3〜8）は、候補ごとの層の札のあいだに掛かる。集約の層の札は、その対象の外に置く。**

| 層 | 札（`Omission.kind`） | 件数の出所 | 1回だけ数える原則 |
|---|---|---|---|
| **集約の層** | `not_indexed`（`pending`/`failed`/`skipped`）、`filtered`（すべての `condition`。`scopeRelation` が `outside_scope` のものも `within_scope` の `decayed` も） | `MemoryStore.aggregateScope` の1回の集約（`docs/recall.md` §2 段0「`filtered` 系 Omission の件数・`not_indexed` の件数は、すべて同じ1回の集約クエリから得る」、ADR 0173） | **掛からない。**スコープ全体について「この条件に当たる記憶が何件あるか」を数えた件数であり、その記憶が候補になって返ったか・候補ごとの札に数えられたかを問わない |
| **候補ごとの層** | `below_threshold`、`over_limit`（`stage: 'rescore' \| 'association'`）、`budget_dropped`、`score_not_comparable`、`unit_assembly_dropped` | 段2〜段4 が候補1件ごとに決めた分類 | **掛かる。**1件の Memory はこれらのうち1つに、最後に落とした段で1回だけ数える。`memories` に返った記憶は、これらのどれにも数えない（決めたこと1） |
| 件数を持たない札 | `stage_skipped`、`ann_truncated`、`ann_unreached`、`lexical_truncated` | — | 件数を持たないので、数え方の問いが立たない |

⟹ 集約の層の札に数えられた記憶が、同じ recall の `memories` に返ったり、候補ごとの札に数えられたりすることはある。これは二重計上ではなく、**層の違う2つの数**である。「`omitted` は返さなかった記憶の集合」（決めたこと1）は、候補ごとの層について成り立つ。

**採った理由**: 集約の層の件数を、候補ごとの結果を見て後から差し引くと、`docs/recall.md` §2 段0 が決めている「件数はすべて単一の集約から取る」とぶつかる——別々の数え方を混ぜると、書き込みのあいだで総和が一致しなくなる、というのがその規律の理由である（ADR 0011 と同じ）。また、語彙で拾われて閾値で落ちた埋め込みの無い記憶について、`not_indexed` と `below_threshold` のどちらを「最後の段」とみなすかも一意に決まらない。`not_indexed` の件数は `ann_unreached` の判定（`eligible = totalInScope - notIndexed`）にも使われている。

**採らなかった案**: 候補になった記憶を集約の層の札から後から差し引く（Issue #1021 の案1・#1025 の案2）。上の理由による。

### 読み手への帰結

- `not_indexed` の件数は「スコープ内で埋め込みが `ready` でない記憶の件数」であり、「そのうち返らなかった件数」ではない。
- `filtered(decayed)` の件数は「スコープ内で減衰しきっている記憶の件数」であり、「そのうち返らなかった件数」ではない。減衰しきった記憶が contested の対向として返ることがある。
- 返した件数と `omitted` の件数を足し合わせて `totalInScope` と比べるときは、候補ごとの層の札だけを足す。

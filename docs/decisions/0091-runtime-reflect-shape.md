...
- **検討して採らなかった案**:
  ...
  3. **`MemoryStatus` / `MemoryEventKind` に `reflected` を足す。**
     ...
     > **追記（2026-09-23、Issue #634）—— 上の「ADR 0089 決定3」は指し先を誤っている。**
     > ADR 0089 の番号付きの決定3は「冪等性は『読んで status で弾く』で買う。専用の
     > 冪等キーを作らない」であり、「ADR 0087 決定1 と同じ3つの理由」「status ゲート
     > 3か所が独立にずれる（ADR 0082 の根）」は、番号を持たない ADR 0089
     > 「検討して採らなかった案」の3番目「`MemoryStatus` に `consolidated` を足す」に
     > 在る。⛔ 本文は書き換えない（`docs/decisions/README.md`）。

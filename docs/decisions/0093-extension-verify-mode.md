...
> 本文の「**これは実際に起きている。`virchamate/virchamate-backend` への導入で、DB オーナーが
> 承認したのは `btree_gin` と `pgcrypto` の2文だけだった。**」は、**この repo に裏づける実測が無い。**
> **【受・未検証】** この ADR は「現物を読んで確認した」「手元で実測したこと」「CI で実測したこと」という
> 書き分けを随所の見出しに使っているが、この一文にはそのいずれも付いていない
> （**先頭の `⭐` は強調記号であって、エビデンス種別の名乗りではない**）。
> ⚠ **外部リポジトリ（`virchamate/virchamate-backend`）の出来事であり、この repo からは辿れない。**
> ⛔ **これは「主張が誤っている」ではない。**⭐ **測っていない、と書くだけである。**
> （ADR 0219 の掃きで残ったもの）

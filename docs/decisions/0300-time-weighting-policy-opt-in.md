...
- **`eval-undated-c1-seat-floor-reinforced`（類型C'）**: 古い予定が `occurredAt`/
  ...
  - **しかし段3a と段3b-1 の間に、この PR 自身が `origin/main` を取り込んでおり
    ...
    文字数からして違う。**
  - 両方の実際のプロンプト・回答を、実 API 無しで突き合わせて確認した。**根拠の出所は
    書式ごとに違う**——旧書式は段3a の生データ
    （`bench-results/answer-time-weighting-stage3a-isolation.json`、プロンプトと回答を
    80 件記録）、新書式は記録済みカセット（`answer-time-weighting.json`）の再生である。
    カセットは段3b-2 で録ったため新書式しか含まず、旧書式をカセットから再生することは
    できない:
    - 旧書式（段3a、生データ）: `- オフィスの座席は3階です。` / `- 先週、座席が5階に移動した。`
      だけを見せると、`eventAwareFreshness` の fail はすべて **「分かりません。」**
      （temperature=0 で 20/20、未指定で 13/20）——`accept`（`5階`）も
      `reject`（`3階`）も含まないため `gradeAnswer` は `fail` になる（LLM が「答えない」
      ことを選んだのであって、古い方の答えを選んだ回は 0 件）。
    - 新書式（段3b-1/3b-2、カセット再生）: 各行に `[出来事時刻:不明]`（古い予定側）/

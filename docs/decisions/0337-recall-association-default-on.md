...
とは言えない。** 符号検定の p 値は食い違った対の数（ここでは5）だけで決まり、
5対が全部同じ向きでも最小値は `2 × (1/2)^5 = 0.0625` である。

**⚠ この p 値は独立性を過大に見積もっている。** 食い違った5対は、すべて同じ
1組（`dev-a2-remote-work-day/legacy`、同じプロンプト）の5回の試行であり、
互いに独立な5件の証拠ではない。試行の揺れではなくケースの違いを単位にすると、
得られた証拠は「6組中1組で正誤が変わり、その向きは on が正」の1件である。
ケース全般について on/off のどちらが正答率を上げるかは、この測定からは言えない。
...
- `dev-a2-remote-work-day/legacy` の1組だけが、5/5 という試行内で完全に
  ...
  70%程度でも5/5が偶然出る確率は無視できない）。生データを見ると、off の
  5回はすべて「分かりません。」、on の5回はすべて「（在宅勤務は）毎週火曜日と
  決まっています。」だった——off では正解の記憶がプロンプトに載っておらず、
  連想枠がそれを載せた形である（`measure-run.json` の `realApi`）。
- **「余計な記憶に引っ張られて誤答した」例は、この60回には無かった**（off正・
  on誤 = 0件）。ただし対象は6組だけであり、無いと言える範囲はこの6組に限る。

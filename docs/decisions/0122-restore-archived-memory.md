...
**決定4（`decay_floor_at` は動かさない）は ADR 0147 が覆した。** `restoreArchived` は
...
足したくなる圧力を生む）は、**ADR 0147 が recall() に既定 ON の忘却ゲートを導入した

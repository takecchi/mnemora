...
## 追記（2026-09、[ADR 0153](./0153-recall-decay-floor-gate.md)、Issue #196）

**決定4（`decay_floor_at` は動かさない）は ADR 0153 が覆した。** `restoreArchived` は
今後、`status` の復帰に続けて `MemoryStore.reinforce` も呼ぶ。

上の「なぜ代行しなかったか（検討した代替案）」が却下した理由（「復帰」と「強化」は
呼び出し側にとって別の意思決定である／将来 `reinforce?: boolean` のような分岐を
足したくなる圧力を生む）は、**ADR 0153 が recall() に既定 ON の忘却ゲートを導入した
ことで、根拠そのものが変わった。** `sweepArchive` の選定条件（`decayFloorAt <= now`）
と忘却ゲートの述語（`decayFloorAt > now` を満たさなければ除外）は**同じ列を見ている**
——`restoreArchived` の対象は定義上すべて忘却ゲートが除く側に居るため、`reinforce` を
代行しない限り、復帰させた Memory は既定では recall に二度と現れない（本 ADR の
「これが覆るとしたら」が「復帰は強化を兼ねるべきだとオーナーが決めたとき」として
既に想定していた条件が、ADR 0153 によって実際に満たされた形である）。

**この節は本文を書き換えない**（履歴を書き換えない）。決定4・「検討した代替案」・
「引き受けた負債」1番・「確かめていないこと」最後の1点（recall の既定挙動を変えない
という前提）は、いずれもこの追記により現在の実装とは一致しなくなったが、
**当時の判断の記録として本文はそのまま残す。** 理由・実装・歯の詳細は ADR 0153
「`restoreArchived` と忘却ゲートの相互作用」節を参照。

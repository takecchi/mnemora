...
`07:21:50Z`）だった。**新パラメータでの同 job（PR #746、run 36109832597、
このラウンド化を push した後の run）は 49秒**（`07:51:53Z` 〜 `07:52:42Z`、
いずれも `gh api repos/takecchi/mnemora/actions/runs/<run>/jobs` で実測）——
**+4秒（約9%増）** に留まった。手元で見えた25〜30%増よりずっと小さい——
job 全体の所要時間には checkout・Node/pnpm セットアップ・依存インストール・
拡張作成という固定コストが乗っており、テスト本体の増分（数秒）がその固定コストで
薄まったためと考えられる（【推論】。固定コスト単体の時間を分離して測ってはいない）。

...
> **追記（2026-09-23、Issue #634）—— 上の「ADR 0215 決定4」は指し先を誤っている。**
> ADR 0215 の番号付きの決定4は「終了コードは足さない —— 判定不能は `2`（pending）に
> 寄せる」であり、下限をどこから取るかには触れていない。「下限を実測側/外部の集合から
> 取り、ハードコードしない」という話は、番号付きの決定1「`verdict()` は、期待する集合を
> 引数で受け取る」（`requiredContexts` を省略可能にしない）と、番号を持たない
> 「案D（採用）: 下限を branch protection の `required_status_checks.contexts` から取る」
> 節に在る。⛔ 本文は書き換えない（`docs/decisions/README.md`）。

...
- **⭐ 生き残った変異と、その理由（段1と段2が互いを庇っている）**:
  ...
  **⚠⚠ この表は 2026-09-07 時点の測定である。**下の「**生き残る**」は**当時本当に
  測った値であり、書き換えていない**——塗り替えると、なぜ PR #173 を書いたのかという
  根拠そのものが消えるからである（「元から歯が在った」ように読める文書になる）。
  **⟹ この2件は 2026-09-13 に両方とも塞がった。**末尾の
  **「その後（2026-09-13）」節**を必ず併せて読むこと。
  ...
  **⟹ ⚠ この「別の作業として残す」は、2026-09-13 に PR #173 で果たした。上の文は
  当時の判断としてそのまま残す。**
  **🔑 覆した理由は「不可能だと思っていたが実は可能だった」ではない**——本 ADR 自身が
  「**それは別の作業として残す**」と書いているとおり、これは**範囲の選択**であって、
  **塞げないという測定ではなかった。**しかも**処方は本 ADR が既に書き終えていた**
  （「段1がわざと絞らない `VectorStore`」）——#173 はそれをそのまま実装しただけである。
  ⟹ 詳細は末尾の「その後（2026-09-13）」節。
...
## その後（2026-09-13）—— 生き残り2件（変異5・変異6）は、両方とも塞がった

**⛔ 上の変異試験の表（「生き残る」）は書き換えていない。**あれは 2026-09-07 時点で
実際に測った値であり、塗り替えると **なぜ PR #173 を書いたのかという根拠そのものが消える**。
⟹ 元の測定はそのまま残し、その後を以下に継ぐ。

⚠ **本 ADR の記録は [ADR 0059](./0059-period-in-ann-stage.md) と形が違う**
（0059 は結果欄に `🔴 緑（生き残る）` / `0本` と書いているが、本 ADR は単に「生き残る」）。
⟹ **無理に 0059 の形へ寄せず、本 ADR の形のまま「いつ測ったか」と「その後」を添えた。**

| 上の表の行 | 生き残っていたもの | 塞いだ PR | 置かれた歯 |
|---|---|---|---|
| **変異5** | 段2の除外集合を常に空にする（`new Set<string>()`） | **#173** | `packages/core/src/__tests__/recall-exclude-provenance-filter.test.ts` の `ExcludeProvenanceStrippingVectorStore` を使う describe |
| **変異6** | `FakeVectorStore`（段1）の除外を壊す | **#173** | 同ファイルの「段1が over-fetch の窓（k'）を無駄にしない」describe |

**処方は本 ADR が既に書き残していたものである**——「段1がわざと絞らない adapter（あるいは
段1の filter から当該フィールドを意図的に外したテスト専用の `VectorStore`）を使って
段2だけを検査する歯」。**[ADR 0059](./0059-period-in-ann-stage.md) の同じ穴を `period` に
ついて塞いだ PR #73 の `PeriodStrippingVectorStore` が、そのまま雛形になった。**
`FakeVectorStore` を薄く包み、`search()` に渡ってきた `opts.filter` から
`excludeProvenanceKinds` **だけ**を剥がして委譲する（他のフィールドは素通し）。
**⛔ 本番コードは1文字も弱めていない。**ラッパは test ファイルの中に閉じている。

### ⭐⭐ 「段1と段2が互いを庇っている」は、2本の独立した歯で解いた —— 4マス全部を測った

**本 ADR の核心の観測は「片方を壊す変異では観測できない」だった。**⟹ それを解くには
**段1だけを測る歯**と**段2だけを測る歯**が**互いに独立**でなければならない。
**⚠ いちばん危ういのは「1本の歯が両方をまとめて守っているように見える」形である**
——それだと片方が壊れたときにどちらが壊れたのか分からない。
⟹ **対応が1対1であることを、対応表の全マスを測って示した。**

**測り方（名乗り）**: **変異はすべて手で当てた**（`.claude/skills/mutation-testing/` の
ハーネスは使っていない ⟹ **足場対照の差し引きは無い**）。各変異は1本ずつ当て、
**当てた直後に `git diff` で木に載ったことを確認してから**走らせた——**当たっていない
変異は必ず緑になるので、確認を省くと「変異を入れたのに緑だった」と誤報告する。**
測り終えたら**その1ファイルだけ**を名指しで復元し、`git status --porcelain` が空である
ことを確認した。コマンドの逐語: `pnpm --filter @mnemora/core run test`。
**基準線: `Test Files 37 passed (37)` / `Tests 528 passed (528)`（全緑）。**
⚠ **この 528 は 2026-09-13 時点の `packages/core` 単独の数であり、上の表の 298 とは
別物である ⟹ ⛔ 引き算して比べてはならない。**

| 壊す場所 | **段1の歯**（新規） | **段2の歯**（新規） |
|---|---|---|
| **段1だけ壊す**（`FakeVectorStore` の exclude 適用を削除。変異6 の族） | **🔴 赤** — `expected [] to have a length of 3 but got +0` | **緑のまま** |
| **段2だけ壊す**（`recall-runtime.ts` の除外集合を `new Set<string>()` に。**変異5 そのもの**） | **緑のまま** | **🔴 赤** — `expected [ 'excluded-inferred-1', …(3) ] to not include 'excluded-inferred-1'` |

**🔑 対角が赤・非対角が緑。**⟹ **2本の歯は独立しており、どちらも当てすぎていない。**
どちらの変異でも **1 failed / 527 passed = 528**（基準線と一致）——歯が走る前に木が
壊れていない（変異が型検査や import を落として早期終了させていない）ことの証拠である。

**⭐ 赤くなってはいけない変異（等価変異）**: 段2の `excludeKinds.has(x)` を
`Array.from(excludeKinds).includes(x)` に書き換える（ふるまいは変わらない）
⟹ **528 本とも緑。**⟹ **この歯は「性質」を測っており、「式の書き方」を測っていない。**
ふるまいを変えない変異で赤くなる歯は、リファクタリングのたびに嘘の赤を出す。

**⭐ 変異6 の族について、もう一段深い観測**: 2026-09-13 に `FakeVectorStore` の exclude
適用を**丸ごと削除**したところ、**#173 以前は 527 本が1本も赤くならなかった**
（本 ADR は変異6 を「向きの反転」で測っていたが、**削除でも同じく生き残る**）。
⟹ **既存の段1の歯は「`filter` に載るか」という*配線*しか見ておらず、
「段1が実際に落とすか」は誰も測っていなかった。**#173 の段1の歯は、まさにそこを埋める
——**除外対象の候補（distance 0）が over-fetch の窓 `k'` を埋めても、除外対象外の候補
（distance > 0）が返る**という形で、「段1が実際に落とす」ことだけを測る
（段2は生かしたままである点が肝——**段2が生きていても、段1が絞らなければ窓が埋まって
0件になる**）。

### ⚠ この再測定が測っていない範囲

- **DB 段は走らせていない**（`packages/core` 単独のスイートだけを回した）。上の表の
  2026-09-07 の測定とは条件が違う。
- **`InMemoryVectorStore`（testkit）・`PostgresVectorStore` の層には撃っていない。**
  本 ADR が記録したとおり、それらの適合テストは同じ変異を既に捕まえる。
- **「段1と段2を*同時に*壊す変異」は撃っていない**（本 ADR が記録した D5 が赤くなる形）。
  ⟹ そこは 2026-09-07 の測定のまま。

...
1. ~~**`--match-head-commit` を実際に渡し、head が変わった状態で `gh pr merge` を実行して
   ...
   ⭐ **2026-09-17 解消（PR #397 のマージ時に実測）。**下記「2026-09-17 追記」を見ること。
...
- ~~**`--match-head-commit` に食い違う sha を渡して実際に `gh pr merge` を実行し、
  ...
  対して再現していない。**~~ ⭐ **2026-09-17 解消。下記「2026-09-17 追記」を見ること。**
...
## 2026-09-17 追記: 食い違う sha で実際に落ちることを実測した

**本 ADR は「引き受けた負債」1番と「確かめていないこと」1番で、
`--match-head-commit` に食い違う sha を渡したときに本当に落ちるかを
実地に再現していないと明記していた。**それをマージする側が実測したので、記録する。

**⚠ 本文の値は書き換えていない**——当時この ADR が「確かめていない」と書いたことは、
その時点の記録として正しい。この節が足すのは、その後に取られた実測だけである。

### 何をしたか 【実測】

**PR [#397](https://github.com/takecchi/mnemora/pull/397)（Issue #256、head = `a68af64`）を
マージする直前に、わざと食い違う sha を渡した。**

```
$ STALE=$(git rev-parse origin/main)   # 67fd5b72b57ccf8dfc91a856f74168436c8ab7c1（PR head ではない）
$ gh pr merge 397 --squash --delete-branch --match-head-commit "$STALE"
GraphQL: Head branch was modified. Review and try the merge again. (mergePullRequest)
$ echo $?
1
$ gh pr view 397 --json state -q .state
OPEN
```

**⟹ `gh` は exit `1` で落ち、PR は `OPEN` のまま残った。**
続けて、緑と判定した正しい sha を渡すとマージされた:

```
$ gh pr merge 397 --squash --delete-branch --match-head-commit a68af64580358422c49e751692b22e44abb4c376
$ echo $?
0
$ gh pr view 397 --json state -q .state
MERGED
...
1. **この歯は実際に噛む。**「決定」2番の主張（見た sha 以外はマージさせない）は、
   説明文の読みではなく実測に裏づけられた。
2. ⭐ **`gh pr merge --match-head-commit` は `docs/autonomy.md` §4.1 の
   「静かに失敗する道具」の族ではない**——失敗は **exit 非0** と
   **エラーメッセージ**の両方で表に出る。⟹ 判定と同じ行に繋いでも
   fail-open しない（ただし §4.1 の ⭐「副作用のある手を判定と同じ呼び出しに繋がない」は
   この道具についても引き続き守るべきである——**理由が「静かに失敗するから」から
   「判定と副作用を分けるのが規律だから」へ変わるだけ**である）。

### ⚠ これでも確かめていないこと

- **「head が force-push で書き換わった場合」と「新しい commit が積まれた場合」を
  `gh` が区別するか**は、依然として確認していない。上の実測が再現したのは
  **前者でも後者でもない第三の形**——`origin/main` という無関係な sha を渡した場合——である。
  ⟹ 「一致しなければ落ちる」ことは示せたが、**不一致の内訳による挙動の差は未検証**のまま。
- エラーメッセージ（`Head branch was modified.`）は GitHub の GraphQL API が返す文言であり、
  **`gh` のバージョンや API の変更で変わりうる。**終了コードのほうを判定に使うこと。

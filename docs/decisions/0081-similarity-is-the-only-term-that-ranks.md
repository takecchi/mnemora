...
## 🔴 訂正の追記: §5 の「11箇所」は **10箇所**である（2026-09-13）

**出所: この追記の担当者が数え直した。**§5 は「渡している箇所は **11箇所**在る」と書き、
内訳を `observe-occurred-at.test.ts` 8 ／ `backfill.ts` 2 ／ `time-term-arm.ts` 1 の表にしている。
**この表は、自分の数え方を行ごとに変えてしまっている。**

### 数え直した（HEAD `0dd2c440f1ce2e18721f7065c86b425d216ba82f`）

⚠ **まず、対象ファイルが動いていないことを確かめた。**
`git diff --stat 01d352d 0dd2c440 -- packages/core/src/__tests__/observe-occurred-at.test.ts`
は**空**である。**⟹ §5 の測定時 HEAD `01d352d` から1バイトも変わっていない。
数の違いは、時間の経過ではなく数え方から来ている。**

| ファイル | `observe()` 呼び出し総数 | うち `occurredAt` を**渡している** | §5 の表 |
|---|---|---|---|
| `packages/core/src/__tests__/observe-occurred-at.test.ts` | **8**（83, 89, 103, **121**, 194, 200, 260, 266行） | **7** | 8 ← ⛔ |
| `examples/chat/src/backfill.ts` | **4**（113, 120, **130**, **136**行） | **2** | 2 ← ✅ |
| `examples/chat/src/time-term-arm.ts` | 1（227行） | 1 | 1 ← ✅ |
| **合計** | 13 | **10** | 11 |

**⟹ 食い違いは1箇所、`observe-occurred-at.test.ts` の行である。**
そこに在る8つの `runtime.observe(` のうち、**121行目は `occurredAt` を渡していない**——
その `it` の題は逐語で
**「渡さなければ `Memory.occurredAt` は null のまま（既定は変えていない）」**であり、
**渡さないことを主張するための対照**である。

**🔴 そして同じ表の `backfill.ts` の行では、同じ除外が正しく行われている。**
`backfill.ts` には `observe()` が4つ在り（`withCtx` 側2つ・`withoutCtx` 側2つ）、
§5 はそこから渡している2つだけを採って**「2（`withCtx` 側のみ）」**と注記している。
**⟹ 除外する規則は分かっていた。適用が1行だけ漏れた。**

### 訂正しても、§5 の結論は動かない

**⛔ 変わるのは数だけである。**§5 の主張
（ADR 0033 §4 の「0件」は現 HEAD では真ではない／
「原理的に不可能」ではなく「このベンチが通していないだけ」／
ADR 0033 §4 の欠陥は格下げされたが解消されていない）は、**10箇所でもそのまま立つ。**

### ⭐ ただし、10箇所の**性質**は書き足す価値がある

| 層 | 箇所 |
|---|---|
| 歯（境界検査） | **7** |
| デモ・測定用 arm（`backfill.ts` 2 ／ `time-term-arm.ts` 1） | **3** |
| **製品コードの呼び出し側** | **0** |

**⟹「経路が10箇所在る」は「製品が `occurredAt` を使っている」ではない。**
[ADR 0037](./0037-callers-pass-occurred-at.md) が
**「直したのは『呼び出し元が0件』のほうであって、機構ではない」**と書いた状態が、
いまも正確にそのまま続いている——**開いた口を通しているのは、歯とデモだけである。**

**⚠ この追記は本文を1文字も書き換えていない**（`git diff` の削除行 0件で確かめた）。
数の訂正を本文へ直接当てると、§5 が「何をどう数えたか」の記録が消える——
**間違え方のほうが、正しい数より情報量が多い。**

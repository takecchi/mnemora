# ADR 0160: `examples/chat` に訂正シナリオを足す — `contestedPair` は構造としての宣言、判定はしない
...
させたため、当初この ADR に振っていた `0158` が衝突した——**`0160` へ採番し直した**
（`0159` は本 ADR 執筆時点で open の PR #325 が名乗っており、既知の衝突を避けるため
あえて飛ばした。下記「確かめていないこと」参照）。以下は `origin/main` をこのブランチへ
merge し、ADR を `0160` へ改名した**後**に実行した最終値である（マージ前の途中経過の数値は
上書きしていない——このセクションの直前の値は、マージ前に実際に測った数値として残す）。
...
- **`0160` が最終的に衝突しないこと。** 番号を確定した時点（`git fetch origin main`
  直後）で `main` の最大は `0158`（PR #323/association-probes-bench）、open の
  PR #325 が `0159` を名乗っていた（`docs/decisions/0159-omission-kind-generation-registry.md`）。
  それを避けて `0160` を選んだが、`docs/autonomy.md` §4「ADR 番号の衝突」が明記する
  とおり**番号は PR を出す直前に確定するのが定石**であり、この PR がマージされるまでの
  間に他の並行 PR が `0159`/`0160` を消費する・あるいは #325 がマージされずに `main`
  の実際の最大が変わる、といった事態は確認していない。マージ担当者が
  `git fetch origin main` してから `node scripts/generate-adr-index.mjs`/
  `adr-duplicate-number.test.mjs` で最終確認することを前提にしている。

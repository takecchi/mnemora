...
## 追記 (2026-09-10): 「これが覆るとしたら」の第1項が満たされた

**⚠ 状態欄は書き換えていない。**この repo に「採用済みの ADR を覆った」と示す書式が無いため
（ADR 0064 が ADR 0063 の数値を訂正したときと同じ扱い）。**決定そのものは今も有効である**
——`compare` は量だけを測り、質の主張を持たない。**変わったのは負債のほうである。**

[ADR 0088](./0088-retrieval-quality-measured-in-ci.md) が `retrieval` を CI のジョブに載せた。

**上の「引き受ける負債」が前提にしていた「`retrieval` は本物の API キーが要るため
CI には載っていない」は、もう成り立たない。**[ADR 0051](./0051-recorded-provider-cassette.md)
のカセット（実 API の埋め込み 152 件）を `MNEMORA_PROVIDER_SOURCE=recorded` で再生すれば、
**鍵なしに実 embedding の想起の質を測れる**——追加コスト 0 という意味で、
「これが覆るとしたら」の第1項が言う**「安価な embedding provider」に文言どおり一致する。**

**⚠ ただし覆ったのは半分である。**

- ✅ **「CI はそれを検知できない」は覆った。**スコアリング式や ANN の段を変えれば、
  arm B / C の MRR・`hit@1`・`hit@10` が動き、その差分が Job Summary と
  基準値ファイルの diff に現れる。
- ⛔ **「検知する歯」にはなっていない。**ADR 0088 は**門を1つも置いていない**
  （標本が probe 7 件しかなく、閾値の門は偽陽性を出す。ADR 0033 §3）。
  **⟹ 最後は人の注意力に残る。**

**⛔ そして「`compare` に質の主張を戻す」ことはしていない。**この ADR の決定は維持する。

---

## 追記 (2026-09-16): 決定2 の理由を差し替えた（ADR 0146、Issue #263）

**⚠ 状態欄は書き換えていない。**上の追記 (2026-09-10) と同じ理由——この repo に
「採用済みの ADR を覆った」と示す書式が無いためである。**決定そのものは今も有効である**
——`compare` は量だけを測り、想起の質の主張をここに載せない。**変わったのは理由のほうである。**

[ADR 0133](./0133-compare-baseline-and-gate.md)（2026-09-15）が実測した通り、`compare` は
もう `deterministic`（擬似 provider）では走らない——`llmMode`/`embeddingMode` はどちらも
`recorded`（記録した実 API 応答の再生）である。**⟹ 上の「文脈」節・「決定」節が言う
『擬似 provider』は、この ADR が書かれた 2026-09-05/06 時点の事実であり、それ自体は
書き換えない**（このリポジトリの「履歴を書き換えない」規律）。**ただし、いま現在形として
「`compare` は擬似 provider だから質を主張しない」と読むと誤りである。**

[ADR 0146](./0146-compare-quality-claim-reason-replaced.md) が理由を検討し直した。要点:

- **線は provider の層（`deterministic`/`recorded`/`openai`/`local`）には引かない。
  bench が正解集合(ground truth)を持つかどうかに引く。** `compare`（`scenario.ts` の
  `buildConversation` + 単一の `FACT_STATEMENT`）は正解集合を持たず、`MRR`/`hit@k` を
  定義できない。`retrieval`（`probe-set.ts`、probe ごとに gold/distractor を持つ）は
  正解集合を持つ。**⟹ 同じ `recorded` 層で、片方(`retrieval`)が質を主張してよく
  片方(`compare`)がいけない理由は、層ではなく正解集合の有無である。**
- **`AGENTS.md` が言う「記録した時点のものだから割り引く」は、`compare` に固有の理由
  ではない**——`retrieval` のカセットにも同じ割引きが掛かる。**この理由だけを新しい
  根拠にすると ADR 0088 と線が引けなくなる**ため、正解集合の有無を主・こちらを従とする
  2本立てにした。
- **決定2 を撤回する案（`recorded` の `compare` から想起の質を主張してよいとする）は
  検討した上で落とした。**北極星の「迷ったときの問い」3（この記憶が選ばれた理由を、
  後から説明できるか）に当てると、`compare` には正解集合が無いため「なぜこの結果を
  質の証拠と呼べるのか」の根拠を説明できない。詳細と、この判断が覆る条件は
  [ADR 0146](./0146-compare-quality-claim-reason-replaced.md) を見ること。

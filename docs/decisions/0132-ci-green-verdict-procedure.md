...
> 本文末尾の Issue #294 の節は「**PR #283 で【実測】として次を踏んだ**」で始まるが、
> その括弧内の「**一過性の corepack `ECONNRESET` で、再実行すれば緑に戻る性質のものではあった**」
> の部分は **【実測】ではなく【受】** である——**赤くなったという事実**と、
> **その赤が「一過性で、再実行すれば戻る性質」だったという原因の性質判定**は別の主張であり、
> 後者は [Issue #294](https://github.com/takecchi/mnemora/issues/294) 本文からの報告である。
> [ADR 0191](./0191-ci-green-verdict-bound-to-sha.md) は同じ記述を「確かめていないこと」に
> 【受】として正しく名乗っている（**3文書とも同じ commit `6fc2afa` / PR #385 で入り、
> 名乗りは 0191 にしか残らなかった**）。
>
> ⛔ **「再実行すれば通る」を一般の性質として裏づける実測は無い。**
> ⭐ **【実測 2026-09-17】** corepack の `ECONNRESET` で落ちていた別の run（`de5ed1c` / run `34625143005`）を
> 1回だけ再実行し、**同じ `registry.npmjs.org` からの取得を踏み直した上で**緑になった
> （[Issue #483 の追記](https://github.com/takecchi/mnemora/issues/483#issuecomment-5710601764)）。**n=1。**
> ⛔ この1本は `f69e4f24` **ではない**。`f69e4f24` 自身は再実行していない。

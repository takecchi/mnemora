...
## 追記2（2026-09-24、§7 の判定の訂正——対照は別の記憶集合で回っていた）

この追記は、自動化された担い手（マネージャーのセッション）が書いた。オーナー本人の判定ではない。上の本文と追記（§1〜§10）は書き換えず、事実の訂正をここに積む。

### 11. `schedule-change-meeting-day` の退行は、main のカセットの記憶集合では消えていない【実測】

§7 は「記録順（候補B）で退行が消えた」と判定した。しかしその対照は、PR #698 が録り直して main に入れたカセットとは**別の抽出で得た記憶集合**の上で回したものだった。PR #698 の録り直しでこのケースが ✅ だったのは、1回だけの試行による。

main（`6408511`。カセットは `64432be` でも同一）の `examples/chat/cassettes/answer.json` にある dev 6件の記憶経路の回答プロンプトを材料にした。同じ行から3通りの描画を作り、回答モデル gpt-4o-mini（temperature は既定）に答えさせた。数えたのは一次判定 `gradeAnswer` の件数だけで、回答の文面は見ていない。eval は使っていない。

| ケース | digest のみ | 本 ADR の描画（`[由来][話者][主題][記録順][出来事時刻]`） | 値が無い欄を省く描画（`[主題:なし]`・`[出来事時刻:不明]` を出さない） |
|---|---|---|---|
| `schedule-change-meeting-day`（5回＋10回＝15回） | **8/15** | **3/15** | **0/15** |
| 他の dev 5件（各5回） | 5/5 | 5/5 | 5/5 |

dev 6件の回答プロンプトの文字数の合計は、digest のみ 623、本 ADR の描画 1,497、値が無い欄を省く描画 1,155。

- この記憶集合での訂正側の digest は「定例会議が水曜日に移動した。金曜日は都合が悪くなった。」であり、reject 語の「金曜」を含む。digest のみでも 8/15 と揺れるのは、このためと見られる。
- ⟹ **本 ADR の描画は、このケースで退行している**（8/15 → 3/15）。一方、誤帰属の eval 2件は通っている。退行しているのは、digest のみでも揺れる1ケースに限られる。**オーナーの判断で、PR #698 は revert しない。**
- 値が無い欄を省く描画（枝 `feat/691-answer-prompt-omit-empty-fields`）は 0/15 で基準に届かないため、**採らない**。
- 残る本筋は、訂正関係（この2件の間の矛盾・訂正）を回答プロンプトへ渡すことである。これは core の訂正経路（#197 / #369〜#372）の領分で、Issue #691 に残す。
- 1回だけの試行では、揺れるケースの退行も改善も見えない。同じ記憶集合で n 回まわし、正答数で見る評価の器は、#498 系の Issue #705 で扱う（CI の門にはしない）。

## 追記3（2026-09-25、Issue #691 続きの決定と配線への1段落のポインタ）

この追記は、自動化された担い手（マネージャーのセッションから切り出された担い手）が書いた。オーナー本人の判定ではない。上の本文・追記・追記2（§1〜§11）は書き換えず、ここに新しい ADR へのポインタだけを積む。

追記2（§11）が残した「同じ記憶集合で n 回まわし、正答数で見る評価の器」は ADR 0301（Issue #705）で作られ、その器を使って `schedule-change-meeting-day` の退行に対する描画の候補比較（`recorded`/`digest-only`/`order-sorted`/`order-legend`/`digest-order-legend` の5通り、n=15、dev 6件のみ）を行った結果、`order-legend`（`recordedAt` 昇順への並べ替え＋「記録順」の凡例1行。由来・話者・主題・矛盾候補のタグは維持）を採用し、`buildMnemoraPrompt` の実装をこれに切り替えた。採らなかった案の理由・記憶集合が変わりうることの注記・`answer`/`answer-time-weighting` の再生カセットを新形式へ移行する配線（既存カセットは1バイトも書き換えない）は [ADR 0305](./0305-answer-prompt-order-legend-and-cassette-migration.md) に集約した——数値・配線の詳細はここには複製しない。

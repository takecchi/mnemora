...
## 追記（2026-09-26、[Issue #1024](https://github.com/takecchi/mnemora/issues/1024)）: disjoint 極の N=50・100 の行は、本文が同一の発話を含むコーパスで測っていた——直して測り直した結果、値も結論も変わらない

クローン miku の委譲先が書いた（オーナーではない）。判断はクローン miku。本文と上の表は書き換えていない。

### 何が起きていたか

`buildUtterances` の disjoint 極は subject k に話題 `k mod 10` だけを話させるが、1つの話題の本文は
テンプレート4 × filler 12 = **48通りしかなかった**。⟹ **1 subject あたり48件を超えると、本文まで同一の
発話を繰り返していた**（N=50 で2件、N=100 で52件＝52%）。上の表のうち、**disjoint 極の N=50・N=100 の行は
このコーパスで測った数字である**（#1024）。それ以外の行（disjoint の N ≤ 20、shared の全行）は、1 subject の中で
本文の重複が無い。

### 直したこと

各話題の filler を12語から25語に増やした（4 × 25 = 100通り）。足した13語は既存の12語の**後ろ**に置いたので、
N ≤ 48 の行と shared 極の全行のコーパスは1文字も変わらない（`examples/chat/src/__tests__/subject-crossing-corpus.test.ts`
が、修正前の生成物の sha256 で押さえている）。通し番号を足す案は、語が増えて類似度の分布が変わるので採らなかった。

### 測り直した結果【実測】

条件は上の表と同じ（`local` 埋め込み、`deterministic` LLM、minAffinity 0.8、seedCap 150）。手元の PostgreSQL 17
（initdb、`C.UTF-8`）、2026-09-26。**同じ機械・同じ DB で、修正前のコーパスでも走らせて並べた**（環境の違いと修正の
影響を切り分けるため）。

**`ctx.subjectId` 無しの混在率（disjoint 極）**

| N | 上の表（修正前、元の測定） | 修正前のコーパス（今回） | **修正後のコーパス（今回）** |
|---|---|---|---|
| 50（S=2 / 5 / 10） | 0% / 0% / 1% | 0% / 0% / 1% | **0% / 0% / 1%** |
| 100（S=2 / 5 / 10） | 0% / 0% / 0% | 0% / 0% / 0% | **0% / 0% / 0%** |

- **値は変わらなかった。**同一の本文の重複は、この2行の混在率を動かしていなかった。
- `ctx.subjectId` = 種と同じ: 修正後も**全セル 0%**。`ctx.subjectId` = 種と別: 修正後も 100%。違いは、eligible が
  2件以上に届いた試行の数だけだった（disjoint の N=100 で S=2 / 5 / 10 が 129 / 133 / 135 → 137 / 140 / 144、150試行中）。
- shared 極（コーパスは変わっていない）は、今回も全セルで上の表と同じ値だった。

### 結論への影響

**無い。**「同じ subject に絞れば 0%、別の subject に絞るとほぼ 100%、絞らなければ規模と話題の重なりしだい」は、
修正後のコーパスでもそのまま成り立つ。ADR 0317（案 S）の根拠も変わらない。

### 確かめていないこと

- 測定の出力（`subject-crossing-local.json`）の `commit` 欄は、実際に使ったコーパスを表していない。修正後の実行は
  作業ツリーに修正が入った状態で HEAD が `8a5c9a0` のときに走り、修正前の実行はその後、修正のコミット後に
  元のファイルの写しから走った。どちらのコーパスで測ったかは、この追記の表の列の見出しのとおりである。
- 表の他の行（重複の無い行）は測り直していない。

...
- 【実測】GitHub Actions の Linux runner の既定シェルについて、この ADR を書く過程で
  ...
  **明示した**ときだけ `bash --noprofile --norc -eo pipefail {0}` になる、という
  ...
  検算していない。**
  ⭐ **その後、依頼者がこれを現物に当てた。**【実測 2026-09-21】`ci.yml` の `example-chat`
  ジョブの fingerprint 門ステップが本番のランナーで印字した値は `shell: /usr/bin/bash -e {0}`
  だった（run `35648302199` / job `106494103566`）。⟹ **web の記述のほうが正しく、ADR 0245 の
  逐語の値は誤りである。**その実測を ADR 0245 の**追記1**として足した（⛔ 本文は書き換えて
  いない。⭕ ADR 0245 の決定と判定は変わらない——依存しているのは「既定が `-e` を含むこと」
  だけであり、`bash -e {0}` は `-e` を含む）。 `docs/decisions/README.md` の規律によりADR 0245 の本文は

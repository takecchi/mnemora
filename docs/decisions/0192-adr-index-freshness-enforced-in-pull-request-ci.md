...
### 2. 並行 ADR PR が両方再生成した場合の挙動を、実際には撃っていない

**設計上は安全なはずである**——両者は索引へ1行ずつ「追加」する形なので、3-way マージの結果は和集合＝正しい索引になる。自動解決できなければ GitHub が衝突として止める。⟹ **`main` が黙って古くなる経路は無い**、と読んでいる。

⛔ **だがこれは推論であり、実際に2本を並行させて撃ってはいない。**

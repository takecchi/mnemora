# ADR 0161: `examples/chat` に訂正シナリオを足す — `contestedPair` は構造としての宣言、判定はしない
...
（`docs/decisions/0160-budget-demo-teeth-and-channel-registry.md`）。⟹ 既知の衝突を
もう一度避けて、**最終的に `0161` を採った。**

⚠ **この節の数値は、ADR がまだ `0160` という名前だった時点で測ったものである。**
`0161` への改名は ADR ファイル名と、それを参照する doc コメント／テストの文字列だけを
書き換えたものであり、**測定対象のコードは一切変えていない**——したがって下の数値は
そのまま有効だと考えているが、`0161` へ改名した後に全門を測り直してはいない
...
- **`0161` が最終的に衝突しないこと。** 番号を確定した時点（PR を出す直前の
  `git fetch origin main`）で `main` の最大は `0159`（PR #325 が着地済み）、かつ
  open の PR #330 が `0160` を名乗っていた。それを避けて `0161` を選んだ。
  **`main` に対する重複は `scripts/__tests__/adr-duplicate-number.test.mjs`（PR #318）が
  CI で検出するが、この検査は「他の open PR が同じ番号を名乗っているか」までは見ない**
  ——`0161` を名乗る別の PR が本 PR より先に着地すれば、やはり衝突する。この PR が
  マージされるまでの間にそれが起きないことは確認していない。
- **`0161` へ改名した後に、全門を測り直していない。** 改名で変えたのは ADR の
  ファイル名と、それを指す doc コメント／テスト内の文字列（`ADR 0160` → `ADR 0161`）
  だけであり、実行されるロジックには触れていない。上の「測ったこと」の数値は
  `0160` 時点のものである。改名後の最終的な緑は CI で確認する。

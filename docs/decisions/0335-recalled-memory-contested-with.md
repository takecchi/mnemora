...
## その後（2026-09-27）——「相手が連想枠経由で単独候補になり contestedWith が付かない」状態の解消（Issue #959）

上の「測ったこと」2番が実測した「相手が最終的な結果集合に居ないことを実際に作れたのは、
連想枠経由で単独候補になった場合だけである」という観測は、その後 Issue #959 で
別の角度から取り上げられた——`docs/architecture.md` §0 原則1（争われている主張は、
それを争う相手と必ず同時に提示する）・`MemoryStore` の契約（`contested` を単独で
返してはならない）と、この状態が食い違っていたことが指摘された。

段3（必須の同伴取得、ADR 0043/0136）は `withinLimit` に載った `contested` にしか
対向取得をかけておらず、段3.5（連想枠。本 ADR・ADR 0151）が独自に拾った `contested`
にはその規則が及んでいなかった、というのが原因だった。ADR 0151 の 2026-09-27 追記が
その修理を記録している——段3.5が選んだ `contested` にも段3と同じ必須の同伴取得規則を
かけ、対向が取れなければ Unit ごと落とす形にした。

**この修理の結果、本 ADR が上で観測していた「相手が連想枠経由で単独候補になり
`contestedWith` が付かない」状態そのものが起きなくなった**——連想枠経由で返る
`contested` は、対向を伴う Unit として返るか、Unit ごと落ちるかのどちらかになり、
「対向を伴わない単独の `contested`」という中間状態が無くなったためである。

**本 ADR の上の本文・「測ったこと」2番・負債3は書き換えていない**——2026-09-17
時点で実際にそう観測され、そう設計されていた記録として残す。ここに書くのは、
その観測が指していた状態が Issue #959 でどう解消したかの追記だけである。

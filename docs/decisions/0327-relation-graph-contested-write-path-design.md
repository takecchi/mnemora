...
選択肢を3つに広げ、評価を添えた上で、改めて同じ場所（オーナー確認）に置く。
...
`recall-runtime.ts` の `runRecall` 内、「段3: 矛盾の解決と必須の同伴取得」ブロック
（`contestedNeedingCompanion` を組み立てる箇所）は今日、`c.memory.contestedWithId` の

...
**これをどう読むか（確かめた範囲）**: 粗い試算を2通り置く。(i) 本 ADR 決定2 の
3回の OFF 測定（計 4/18 ≈ 22%）を1機会あたりの真の確率と仮定すると、18機会すべてで
0件になる確率は 0.78¹⁸ ≈ 1% である。(ii) [ADR 0324](./0324-claim-key-contested-detection.md)
§4 の話題別の値（`family` 4/5・`language` 3/5・`diet` 2/5 ≈ 80%/60%/40%）を仮定すると、
この3話題×n=3 の9機会すべてで0件になる確率は 0.2³×0.4³×0.6³ ≈ 0.01% である
（0324 の値は `contested` の件数＝predicate 一致も要るため、誤帰属そのものの率は
これ以上のはず）。**どちらの仮定でも、単なる標本のばらつきだけでこの0/18を説明する
のは苦しい**（ただし (i) の約1%は、偶然を退けるほど強くはない）。
...
- ⛔ **陽性対照 n=3 という回数の限界**——上記「これをどう読むか」で示した同時確率の
  試算（決定2 の値を仮定すると約1%、ADR 0324 §4 の話題別の値を仮定すると約0.01%）は、
  過去の少ない観測値（n=3〜5）を真の確率と仮定した粗い見積もりであり、厳密な統計検定
  ではない。

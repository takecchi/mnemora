...
## 追記（2026-09-25、Issue #766）: 「1文であること」を、テストだけで歯にした（Mu5a を殺す）

この追記は、クローン（miku）の委譲で動くセッションが書いた。オーナー本人ではない（ADR 0220）。
上の本文は書き換えず、ここに実測だけを積む。「引き受けた負債」節の「1文（`UNION ALL` +
`NOT EXISTS`）を採ったことは、歯では守られていない」という項目のうち、**「1文である」
という形だけ**を閉じる。

### 採らなかった道（引き受けた負債が名指しした2つの要素）

「引き受けた負債」は、塞ぐには (1) テスト用の delay-injection フックと、(2) 「1文/2文で
返す行のスナップショットが違う場合、どちらを返すべきか」という本 ADR がしていない新しい
判断、の2つが要ると書いていた。**このセッションはどちらも採らない。**どちらも本体
（`memory-store.ts` の非テストコード）を変えることになり、(2) はこの ADR がまだしていない
設計判断を要するため——このセッションの委任範囲（テストの追加だけで塞ぐ）を超える。

### 採った道: 文の数を数える歯

代わりに、**DB へ実際に送られる文の数を数える**形で、「1文である」という*形*だけを
テスト側だけで固定した:
`packages/postgres/src/__tests__/set-embedding-status-single-statement.postgres.test.ts`。

`test-db.ts` の `captureClientQuery`（ADR 0284）と同じ観測点（`Client.prototype.query`、
`pool.query()` 経由・`db.transaction()` 経由のどちらでも拾える）を使い、`fn()` の実行中に
呼ばれた回数を数える。

- **1本目（本題）**: `ready` の行へ `failed` を書こうとして弾かれる経路（ガードで
  `UPDATE` が0行しか更新せず、`UNION ALL` の読み戻し側が通る経路）で、送られた文が
  1件であることを断言する。**これが Mu5a を殺す本体**——Mu5a の「0行なら別の `SELECT`」
  という追加の分岐は、この経路でしか実際に実行されない。
- **2本目（形の固定、Mu5a への捕獲力は無い）**: 通常の更新経路（`pending → ready`）でも
  1文であることを断言する。成功する更新では Mu5a の追加分岐自体が実行されないため、
  この歯単体では1文でも2文でも変わらない。それでも「今日は1文である」という形を
  固定しておく——将来、成功経路のほうを複数文へ割る変更が来たときに、最初に気付く
  場所になる。

**⚠ 並行時にどちらのスナップショットを返すかは、この歯では一切断言していない。**
上の「引き受けた負債」節が書いた並行性の議論（窓が sub-millisecond で外部から
到達不能）はそのまま変わっていない——**この追記が閉じるのは「1文である」という形だけ**
であり、「どちらのスナップショットを返すべきか」という設計判断はまだされていない。

### 実測（この器: `initdb` で立てた自分専用の Postgres 17 + pgvector）

- **歯を追加する前**（今日どおりの1文の実装）: 新設の歯2本とも緑
  （`pnpm --filter @mnemora/postgres exec vitest run
src/__tests__/set-embedding-status-single-statement.postgres.test.ts`、2 passed）。
- **Mu5a を当てた後**（`memory-store.ts` を `cp` で退避してから、1文を
  「`UPDATE ... RETURNING *` → 0行なら別の `SELECT`」の2文へ割る変異を実際にコードへ
  入れた）: 同じコマンドで**1本目（本題）が赤くなった**
  （`expected 2 to be 1` — 送られた文が2件観測された）。2本目（通常の更新経路）は
  上記のとおり捕獲力が無いため緑のまま——狙いどおりの選択性。
- **`cp` で戻した後**: `git status --porcelain` で `memory-store.ts` の差分が空に
  なることを確認し、同じ歯2本が緑に戻ることを確認した
  （`captureClientQuery` の頑健さの実測と同じ形で、赤くなった原因が狙った歯であることを
  同一 it の緑復帰で確かめた）。
- **`packages/postgres` の `typecheck` / 新設ファイルの `eslint` / `prettier --check`**:
  すべて緑。
- `packages/postgres` の `test:db`（704件）は、この歯以外の1ファイル
  （`trigram-lexical-store.postgres.test.ts`、SQL_ASCII/C ロケール判定）で4件、
  この器の `initdb --locale=C` に起因すると見られる既存の失敗が出た——**この追記の歯とは
  無関係**（調べていない。並行して走っている別の負債の可能性がある）。

### 確かめていないこと

- **並行性そのものは、この追記でも再現していない。**上の「引き受けた負債」節・
  「確かめていないこと」節が書いた限界がそのまま当てはまる。

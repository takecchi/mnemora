...
### 【CI 実測】同じ HEAD の CI では、桁がもう1つ違った

**この PR の `retrieval-quality` ジョブ（GitHub Actions の runner）のログで、§6.1 で足した同じ印字は
`decay=10通り[0.9999998376773517..0.999999901322802]` を出した**——1 からの差は `9.9e-8`〜`1.6e-7`、
幅は `6.365e-8` である。

**⟹ 3つの器で3つの桁が出た**（ADR 0081 の器 ≈ 1e-8 桁 / 本測定の器 ≈ 1e-7 桁 / CI の runner ≈ その中間）。
**⟹ §4 の訂正は、これで裏が取れている。桁は器の速さの関数であり、この系の性質ではない。**

**⭐ そして同じログの同じ probe で、既存の印字（`formatScoreDetail`）はこう出している:**

```
項ごとの何通りか(返った候補全体): … decay=10通り[0.9999998376773517..0.999999901322802] …
#1 [top1] total=0.949673 = similarity 0.949673 × decay 1.000000 × tagMatch 1.000000 × freshness 1.000000 × strength 1.000000
```

**⟹ ADR 0081 §6.2 が記録した欠陥と、その修正が、同じ画面に並んでいる。**
**上の行は10通りだと言い、下の行は両方とも `1.000000` だと言う。⟹ 下の行だけを見て「定数だった」と読むと誤る。**
...
**【実測】この歯と、ADR 0108 の歯を含む同ファイルの5件は、本物の PostgreSQL 17.11 + pgvector 0.8.6
（専用 DB・擬似 provider）に対して実際に走らせ、5件すべて緑（EXIT=0）であることを確かめた。**
...
- **§5.3 の検算（`total` を各行で再計算して厳密比較する）は恒久化していない。**
  §6.1 で器へ置いたのは**通り数・行ごと等価・min/max** であり、**積の検算は今回の測定限りである。**
  ⟹ 「隠れた項が無い」は、次に誰かが同じ検算を足し直すまで、この ADR の記録が唯一の根拠である。

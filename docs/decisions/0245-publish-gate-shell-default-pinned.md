...
### 6. 門ステップの取り出しに、script の**名前**を使わない。**形**で選ぶ

⛔ **最初の実装は「`pnpm run typecheck` を含む `run: |` ブロック」を門ステップとして選んでいた。⟹ 落とした。**

**理由**: それは **script 名を歯へ焼き込むこと**である。`typecheck` という名前は
`main` が動けば変わりうる側であり、`AGENTS.md`「⚠ 数を、道具と生成物に焼き込まない」の
「⭐ 線は「`main` が動くと変わるか」である」に当たる
（⭐ [ADR 0244](./0244-runtime-method-doc-correspondence-tooth.md) 決定2 が、
同じ線を `Runtime` のメソッド集合について引いている）。

🔴 **そして実害が実測で出た。**【実測 2026-09-19】名前を錨にした版で変異試験C
（`pnpm run typecheck` → `pnpm run typechek`）を打つと、**`it` 5（script 名の実在）だけでなく
`it` 2（門ステップの実在）も道連れで赤くなった**——⟹ **「門が消えた」という誤った診断が出る。**
実際には門は在り、script 名が変わっただけである。

⟹ **形だけで選ぶ**: 「`pnpm run` の行が**2本以上**並ぶ `run: |` ブロック」。
【実測 2026-09-19 / `publish.yml`】この形に当たるブロックは**門ステップ1つだけ**である。
⭐ **2つ以上に増えたら `it` 2 が「曖昧」として赤くなる**——⛔ 黙ってどちらかを選ばない。

**【実測】この変更の後、変異試験C で落ちるのは `it` 5 の1本だけになった。**

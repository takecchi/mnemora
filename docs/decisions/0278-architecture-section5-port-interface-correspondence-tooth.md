...
## 🔴 追記4（2026-09-23）—— `:333` の「ADR 0273 決定2」は番号付き決定を指していない
...
**何が壊れているか**: 本文 `:333`「ADR 0269「採らなかった案」・ADR 0273 決定2 が示す通り」
は、[ADR 0273](./0273-architecture-section5-is-a-copy.md) に**番号付きの決定が複数在って
その2番目**という体裁を持たせているが、実際には違う——[ADR 0273](./0273-architecture-section5-is-a-copy.md)
は文書全体を `## 0.` 〜 `## 6.` の通し番号で節立てしており、`## 2. 決定` は
「通し番号2番目の節がたまたま『決定』という表題を持つ」だけで、**その節の中に決定1・決定2…
のような複数の番号付き決定は無い**（単一の決定を1節にまとめている）。

⟹ `ADR 0273 決定2` という書き方は、`ADR 0223` の決定群を着地させた略記（この ADR が
他所で訂正している形）とは別の壊れ方であり、機械的な検査
（`scripts/adr-citation-lib.mjs` の `findAdrDecisionSectionNumbers`）でも
「決定セクションに 決定2 が無い」と判定される。

**正しい指し先**: `ADR 0273 の「2. 決定」節`（通し番号の節を指す形。`決定2` という
数字付き決定を指す形ではない）。

⛔ **本文 `:333` は書き換えない。** 検算のためにここへ書き残す。

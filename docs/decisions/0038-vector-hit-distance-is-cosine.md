...
## 🔴 訂正の追記（2026-09-07、[ADR 0040](./0040-zero-vector-never-returned.md)）

**この ADR の決定2に書いた「pgvector の `<=>` はエラーになる」は誤りだった。**

> 全成分0のベクトルは使わない——コサインが未定義になり、pgvector の `<=>` はエラーになる
> （`packages/postgres/src/bench/scale-bench.ts:667` に同じ注意が実測としてある）。

**実測すると `NaN` を返す。エラーにはならない**（pgvector 0.8.2。両引数位置・両方ゼロ・
`ORDER BY` の中でも例外にならないことを確かめた。詳細は ADR 0040）。

**⚠ 本文は書き換えていない。**この記述は `scale-bench.ts:667` の既存コメントを根拠として
書かれたものであり、**誤った前提がどこから来て、どこへ写ったかを追える形で残す。**
**同じ誤りは3箇所にあった**——`scale-bench.ts:667`（出どころ）・この ADR の決定2・
`in-memory-vector-store.ts` の doc コメント。**ADR 0040 で3箇所とも直した。**

**⚠ この訂正は、決定2の残りの部分（全成分0のベクトルを適合テストに使わない）を覆さない。**
NaN も距離としては使い物にならないので、使わない方針そのものは変わらない。

...
**⚠ この文書で「レビュア」と書いているのは、この repo のマージ判断をしている側であって、
人間のオーナー（takecchi）ではない。**（当初この文書は両者を「オーナー」と一語で書いており、
**「人間が決めた」と読める形になっていた。**次に読む人が出所を取り違えないよう、書き分ける。
**人間のオーナーは、この決定を見ていない。**）
...
**これはレビュアの恥ではなく、repo の危険である。**§2 の通り、原因は出力の形に在る。
...
**レビュアの要求（「2回目の実行で `ingest` の結論が変わらない」ことを歯で測る）を満たさない。**
止まる run は、結論を出していない。
...
レビュアの台帳へ移した。**次は「`k'` はどう決まるか / 35件が落ちるのは設計か事故か」を

...
## 🔴 訂正の追記（2026-09-23）—— `:81` / `:93` の略記を、着地元の番号へ揃える
...
- `:81`「（[ADR 0234](./0234-bake-no-numbers-into-tools-and-artifacts.md) 決定9・…への自己適用）」
  —— [ADR 0234](./0234-bake-no-numbers-into-tools-and-artifacts.md) 自身の決定は1〜4であり、
  決定9は無い（決定9は ADR 0223 のものであり、0234 はその着地先）。
  正しくは **ADR 0223 決定9（ADR 0234 で着地）**。
- `:93`「ADR 0255 決定5 の一般形が求めるのは…」—— [ADR 0255](./0255-tools-output-candidates-not-verdicts.md)
  自身の決定は1〜2であり、決定5は無い（決定5は ADR 0223 のものであり、0255 はその着地先。
  0255 の h1 が「ADR 0223 決定5 の射程を `AGENTS.md` へ広げる」と名乗る通り）。
  正しくは **ADR 0223 決定5（ADR 0255 で着地）**。

⛔ **本文 `:81` / `:93` は書き換えない。** 検算のためにここへ書き残す。

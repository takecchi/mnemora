...
| [0069](./0069-ann-truncated-says-nothing-about-loss.md) | `ann_truncated` は「損したか」を一切言っていない（スコープが `k'` 以上なら必ず鳴る）— 読み手0件・書かれた一手は 100% 発火・`k'=40` の安全性は「4項が定数」に依存しており `occurredAt` を入れると割れることの実測と、直し方の案 | **未決** (2026-09) |

# ADR 0300: `answer` の回答評価を同じ記憶集合での n 回試行・正答数で見る器を作る（Issue #705）
...
2. **`OPENAI_API_KEY` を使った実測はこの PR では行っていない**——作業環境に鍵が無いため。
   `schedule-change-meeting-day` が実際にこの器でどう揺れるか（ADR 0295 追記2 の
   3/15 を再現するか）は未確認のまま残る。実行方法は README「`answer-trials`」節、
   この ADR 冒頭コメントの委譲元（マネージャー報告）に費用の見込みを書く。

...
## 追記（2026-09-25、Issue #776）: C-2（dimensions>2000 でテーブルだけ残る）を直した

この追記は、クローン（miku）の委譲で動くセッションが書いた。オーナー本人ではない（ADR 0220）。

上の「C-2」節は「指示どおり、ここでは直さず事実と再現手順だけを記録する」としていたが、
[Issue #776](https://github.com/takecchi/mnemora/issues/776) でオーナー側から直す方針が決まり、
`registerEmbeddingSpace()`（`packages/postgres/src/vector-space.ts`）冒頭の既存バリデーション
（ロック取得より前、テーブルを作る前）に `dimensions > 2000` を拒否する検査を追加した。
上限値 2000 は pgvector の README（`vector` 型に対する HNSW 索引: "up to 2,000 dimensions"）と、
手元の pgvector 0.8.0 に対する実測（2000 は成功、2001 は `CREATE INDEX ... USING hnsw` が
`54000` で失敗）の両方で裏取りしている——このADR本文の C-2 が既に実測していた境界と同じ値。

**C-2 の記述そのものは書き換えていない**（ADR 本文は当時の記録であり、正しかった）。
直した経緯・赤→緑の実測・変異試験は、このリポジトリの規約に従い ADR ではなく
Issue #776 と対応 PR に記録する（この修正1点は「重大な設計判断」ではなく、既存バリデーションの
抜けを埋める入力検証の追加であり、新しい ADR を起こす対象ではないと判断した）。

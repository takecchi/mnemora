...
> **⚠ 2026-09-24 追記（本文は書き換えていない）: 上の表の 0260〜0263 の4本は、2026-09-24 に「採用」へ倒した。**
>
> 根拠は、オーナーの 2026-09-24 の指示（承認キュー経由・repo の外）で、推奨案（状態欄は「採用」へ倒し、未読の名乗りは各本文の冒頭に残す）での採用が承認されたことである。
> ⚠ **これは下の「これが覆るとしたら」の「オーナーが『未読は状態欄で表す必要はない』と答えたとき」への逐語の答えではない。**依頼者（クローン）が承認を「推奨案でよい」と伝え、担い手がそれをこの条件に当たると読んだ。⟹ **未読という事実は状態欄からは消えるが、各本文の冒頭の「本文の逐語は読んでいない」の名乗りと、その下の 2026-09-24 追記に残る。**
> 倒し方は決定2 と同じ形（状態行だけを置き換え、本文と冒頭の名乗りには触らない）。倒した時点で、4本の本文にも決定3 が掛かる。

...
4. **K を「`recall()` の既定 `limit`（=10）」にした**。実測順位（1,1,2,6,1,1,2）
   ...
   > **追記（2026-09-23、Issue #634）—— 上の「ADR 0201 決定3」は指し先を誤っている。**
   > ADR 0201 の番号付きの決定3は「CI ジョブは増やさない」であり、較正係数
   > （`charsPerDigest / 2`）から FLOOR を導いた話は、番号を持たない
   > ADR 0201「なぜ FLOOR を」で始まる節の「案A（採った）」に在る。実測値を固定すると
   > 正当な変更で意味なく赤くなる、という理由づけは同じ節の「案B（検討したが採らなかった）」の
   > 却下理由である。⛔ 本文は書き換えない（`docs/decisions/README.md`）。この歯の docstring
   > （`examples/chat/src/__tests__/retrieval-quality-regression.postgres.test.ts`）の同じ誤引用は、
   > 生きたコードなので指し先を直した。

# ADR 0163: `examples/chat` が `memory_usage` を報告する — `reinforce` を実アプリで発火させる（Issue #301）
...
3. **⛔ `ComparisonRow.memoryUsageReported` を `compare-json.ts` の
   ...
   > **追記（2026-09-23、Issue #649）—— 上の「ADR 0133 の決定4」は指し先を誤っている。**
   > ADR 0133 の番号付きの決定4は「`.github/workflows/ci.yml` の `example-chat` ジョブに
   > 配線する」であり、門の判定基準には触れていない。「全欄一致を門にしない」という中身は、
   > 決定3「⭐ 他5本と異なり、これを門にする」の**判定基準**（2値のどちらかが退行したときだけ
   > 非0で終わり、「それ以外の相違(`naiveChars` の変化・新しい/消えた会話長など)は
   > Job Summary に報告するのみで exit 0 のままである」）に在り、その理由は番号を持たない
   > 「却下した案」の2番目「⛔ 全欄(`naiveChars`/`totalInScope`/`omitted` 等)の厳密一致を
   > 門にする。」に在る。⛔ 本文は書き換えない（`docs/decisions/README.md`）。

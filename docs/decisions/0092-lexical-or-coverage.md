...
**守っているもの・正直な限界**: 600 という値は「普通の長さの自然文クエリ（数百文字）を
削らない」ことを優先して選んだ——**この値だけでは、語数・1語の文字数の上限を
組み合わせた入力を、当初狙っていた水準まで抑えることはできていない**。どれだけ改善
したか・どれだけ届かなかったかの具体的な数字は、この Issue の作業記録（非公開の
ベンチ実測）にのみ残す——本 ADR には数字を置かない。

**採らなかった値**:
- **普通の長文クエリを削らない値を優先しつつ、組み合わせた入力も当初の狙いの水準に
  収める、という両方を満たす値。** 採れなかった——実測の結果、この2条件を同時に
  満たす値は無かった（片方を満たそうとすると、もう片方から大きく外れる）。⟹ 本追記の
  値は「普通の長文クエリを削らない」を優先し、もう一方は達成できていないことを
  正直に記録する。
- **語数の上限（32）をさらに下げる（24・16）ことで、この差を埋める。** 採らない——
  クエリ全体の文字数の上限（600）が先に効くため、その後に残る語の実効的な数は
  16 をも下回る水準まで既に落ちている。語数の上限をさらに下げても、クエリ全体の
  文字数の上限が既に支配的であり、追加の効果は実測上ほぼ無かった。⟹ 語数の上限は
  32 のまま変えない。

確かめていないこと: 600 という値そのものが最終形だという確信は無い——「両方を満たす
値が無い」という結論の前提（測り方・入力の形）が変われば、見直しの余地は残る。

...
- **64。** 採らない——上限として弱すぎる。最悪の入力（テナントの記憶の多くが同じ語を
  含むところへ、上限いっぱいの語数のクエリを投げる）で、DB の接続が長く占有されうる
  ことが実測で分かった（具体的な秒数・入力の形はここには書かない——公開の場所に
  悪用の手がかりを残さない、という本 repo 全体の方針。実測はマネージャーへの報告に
  留めた）。
...
**異なる語の数の上限（32）だけでは防げない形が残っていた。** 空白を1つも含まない
代わりに記号（`-`/`.` 等）だけで長くつないだ「1語」は、語数の上限には触れないまま、
`websearch_to_tsquery`（語の中の記号を区切りとして解釈する）の計算量を語数の上限とは
無関係に膨らませうる——実測でこれを確認した（具体的な文字数・秒数・入力の形はここには
書かない。実測はマネージャーへの報告に留めた）。
...
同じ形で入れた。** `InMemoryLexicalStore` の語の分割（`tokenize`）はもともと記号も
区切りとして扱うため、この特定の抜け道はそもそも無かった——それでも3実装の挙動を
できるだけ揃えるため、同じ上限を同じ形で入れた（`in-memory-lexical-store.ts` の
`LEXICAL_QUERY_MAX_WORD_CHARS` の doc 参照）。

**採らなかった値・考え方**:
- **文字数の上限を置かず、語数の上限だけで済ませる。** 採らない——上で書いた抜け道
  （記号つなぎの1語）を塞げない。
- **1語あたりの文字数の上限を、語数の上限（32語）と掛け合わせた最悪ケース
  （上限いっぱいの語数 × 上限いっぱいの文字数の語）まで見積もって、そちらを
  「〜1秒前後」に収める値にする。** 採らない、または今回は見送った——実測すると、
  32語すべてを上限文字数いっぱいの記号つなぎにした組み合わせは、単独の1語を
  上限文字数まで伸ばした場合よりもさらに悪化する（実測はマネージャーへの報告に
  留める）。この掛け合わせの最悪ケースまで「〜1秒前後」に収めるには、1語あたりの
  上限をさらに切り詰める必要があり、そうすると通常の識別子（UUID・ハッシュ値等）
  まで頻繁に切り詰めることになる。**⟹ 今回は「単独の1語」を測った値で決め、
  掛け合わせの残りうるリスクは残った課題として置く**（マネージャーへの報告に
  数字つきで記録済み。この ADR の「確かめていないこと」にも書く）。

確かめていないこと（追加分）: `PostgresTrigramLexicalStore` の日本語側の文字数の
上限（100文字）は、語数の上限を掛け合わせない前提（日本語側は分かち書きをせず
1つの文字列として扱うため、語数の上限の対象外）で決めた——ASCII 側の語数上限との
掛け合わせは確かめていない。

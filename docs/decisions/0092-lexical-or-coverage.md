...
- **🔴 ADR 0084 §8 の「低選択率のクエリ」の負債は、この PR でも塞がっていない。**
  ...
  **それでも語1つのクエリでは全件が `coverage = 1` で並んでしまうこと**を、
  ...
  > **追記（2026-09-23、Issue #649）—— 上の「ADR 0084 §2 が『simple』を選んだ理由」は
  > 裏づけが見当たらない。**
  > ADR 0084 §2「【実測】正規化した `simple` の再現率と精度」は `websearch_to_tsquery` の
  > 再現率・精度を識別子ごとに測った表であり、「語幹処理もストップワード除去もしない」を
  > `simple` を選んだ理由として述べていない。ADR 0084 本文には「語幹」「ストップワード」
  > という語自体が見当たらない（探した語: 「語幹」「ストップワード」）。
...
**⟹ クエリの異なる語の数に上限（32）を設け、超えた分は先頭から32語だけを使うことにした。**
...
（`capLexicalQueryWords`）、および `packages/testkit`（`InMemoryLexicalStore`）・
`packages/core`（`FakeLexicalStore`）の同じ形の実装。
...
### 追記（2026-09-26、同日中の見直し）: 上限の値を 64 から 32 へ

**上限の値は当初 64 として提起したが、32 に改めた。** 採らなかった値と理由:

- **64。** 採らない——上限として弱すぎる。
- **16。** 採らない——普通の長さの自然文クエリまで削りかねない。上限は「悪意ある/
  壊れた入力を防ぐ」ためのものであり、正常な用途を狭める副作用が大きいと判断した。
- **32（採用）。** 重複をまとめた後に32語を超える自然文のクエリはまれである。
  語彙(lexical)チャンネルはベクトル検索（ANN）を補う側のチャンネルであり
  （`docs/architecture.md` §5.2.1、ADR 0084）、まれな長文クエリで語彙チャンネル側が
  一部の語を切り詰めても、失うものは小さいと判断した。

確かめていないこと: TS 側の「語」の数え方（`capLexicalQueryWords`・
`packages/testkit`/`packages/core` の同種の実装）が、SQL 側（`mnemora_lexical_query_tsqueries`
の `DISTINCT`）の語の数え方と完全に一致することは確かめていない——ずれの向きは
安全側（TS 側が実際より厳しめに切り詰める方向）にしか働かないことは論理的に
説明したが、網羅的な検査はしていない（`lexical-query-cap.ts` の doc コメント参照）。

### 追記（2026-09-26、同日中の見直し）: 語ごとの文字数にも上限を置いた

**語数の上限（32）だけでは、大きな入力を抑えきれない場合があった。** 1語の長さにも
上限を置いた。

**⟹ 1語（空白を含まない、連続した非空白文字の並び）の文字数にも上限を置いた。**
ASCII 側は64文字（`LEXICAL_QUERY_MAX_WORD_CHARS`）、`PostgresTrigramLexicalStore` の
日本語側（非 ASCII の連なり全体、分かち書きをしない1つの文字列として `word_similarity`
に渡す）は別軸で100文字（`TRIGRAM_JAPANESE_QUERY_MAX_CHARS`）。超えた分は先頭から
切り詰める——語数の上限と同じ形（例外にしない、上限に触れない大多数のクエリは
1バイトも変わらない）。実装は `packages/postgres/src/lexical-query-cap.ts`。

**`packages/testkit`（`InMemoryLexicalStore`）・`packages/core`（`FakeLexicalStore`）にも
同じ形で入れた。** 3実装の挙動をできるだけ揃えるため、同じ上限を同じ形で入れた
（`in-memory-lexical-store.ts` の `LEXICAL_QUERY_MAX_WORD_CHARS` の doc 参照）。

**採らなかった案**:
- **文字数の上限を置かず、語数の上限だけで済ませる。** 採らない——語数の上限だけでは
  防げない大きな入力の形が残るため。

確かめていないこと（追加分）: 上限の組み合わせについては、なお見直しの余地がある。

### 追記（2026-09-26、クローン miku の判断）: クエリ全体の文字数にも上限を置いた

**語数（32）・1語あたりの文字数（64/100）の上限を組み合わせても、なお大きな入力が
残っていた。** 「上の追記で残った見直しの余地」の実測から、クエリ全体の文字数にも
独立した上限を置くことにした。

**⟹ `LEXICAL_QUERY_MAX_TOTAL_CHARS`（600）を新設し、クエリ全体の文字数がこれを超える
場合は先頭から600文字に切り詰める。** 他のどの上限（語数・1語の文字数）よりも**先に**
適用する——語への分割・重複まとめ・語数の上限は、この切り詰めの**後**の文字列に対して
行う。超えた分は先頭から使う・例外にしない・上限に触れない大多数のクエリは1バイトも
変わらない、という既存2つの上限と同じ形を保った。

**ASCII 側・日本語側（trigram）は同じ1つの軸**——クエリ全体の文字数の上限は、
`PostgresTrigramLexicalStore` の ASCII 側処理と日本語側処理の両方に、同じ1回の
切り詰め結果を渡す（別々の軸にしない）。日本語側には引き続き別途
`TRIGRAM_JAPANESE_QUERY_MAX_CHARS`（100）があり、クエリ全体の文字数の上限が先に、
日本語側専用の上限がその後に効く。

実装は `packages/postgres/src/lexical-query-cap.ts`（`capLexicalQueryTotalChars`）、
`packages/testkit`（`InMemoryLexicalStore`）・`packages/core`（`FakeLexicalStore`）の
同じ形の実装。

**守っているもの**: 600 という値は、普通の長さの自然文クエリ（数百文字）を削らないことを
条件に選んだ。語数・1語の文字数・全体の文字数の3つの上限は、1回の検索にかかる時間を
**有界にする**ためのものであり、時間そのものの上限ではない。DB 側の `statement_timeout`
と併用する前提であることを、`docs/architecture.md` §5.2.1 と `packages/postgres/README.md`
に記した。一致した行の数と語の数の積に比例する計算の形そのものは残っており、その見直しは
Issue #878 に残した。

**採らなかった案**（2026-09-26、クローン miku の判断）:
- **語数の上限をさらに下げる（24・16）。** 採らない——クエリ全体の文字数の上限が先に
  効くため、語数の上限をさらに下げても追加の効果はほぼ無かった。語数の上限は 32 のまま
  とした。
- **語を数える単位を細かくし、合計の上限を小さくする。** 採らない——普通の長文クエリ
  まで削ることになる。
- **検索の中で時間の上限を掛ける、または coverage の計算の形を変える。** 今回は採らない
  ——前者は新しい失敗（例外）を呼び手に返すことになり、後者は順位を変える。

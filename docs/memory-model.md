...
| 4 | active → active（reinforced） | `observe({kind:'memory_usage', ...})` により `recall_usages` へ新規行が実際に挿入されたとき | 同期（`observe()` と同一トランザクション） | `recall_usages` へ INSERT（新規のみ）、`memories.last_reinforced_at` / `decay_floor_at` 更新（`strength` は動かさない。ADR 0041） | なし（使用の記録は `recall_usages` の行の存在で表す。⚠ **2026-09-26 改訂（[Issue #871](https://github.com/takecchi/mnemora/issues/871)）: この欄は以前 `updated`（`meta.reason='reinforced'`）と書いていたが、実装はどの adapter でも `memory_events` に書かない。**文書を実装に合わせた。理由と採らなかった案は §9 の改訂注記と [ADR 0009](./decisions/0009-usage-feedback-via-observe.md) の追記） |

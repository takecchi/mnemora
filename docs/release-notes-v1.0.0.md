...
7. 🔴 **`v1.0.0` と `v0.5.0` のあいだに commit が在るかを、当日その場で引くこと**（`git rev-list --count v0.5.0..origin/main`）。
   ⚠ **【実測 2026-09-21】0 である**——**下の本文はそれを前提に「コードの差分は無い」と書いている。**
   ⟹ ⭐ **1本でも増えていたら、その一文を直すこと。**
...
> ### 🔴 まず — **`v0.5.0` を使っている方へ**
>
> **`v1.0.0` は、`v0.5.0` とコードが同一です。**【実測 2026-09-21】`git rev-list --count v0.5.0..main` は **0** で、**2つの tag は同じ commit を指しています。**
>

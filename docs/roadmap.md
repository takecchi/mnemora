...
⭐ **追記（2026-09-21）: 下の ⚠3 の留保は解かれた** —— **オーナー（takecchi）の決定（2026-09-19）により、項目4 の「在る」は `association`（段3.5 連想枠）が既定 off であることに依存しない。**🔴 **これはオーナーの決定であって、担い手が技術的に検証して得た結論ではない。**⛔ **以下の本文と ⚠3 は、2026-09-16 / 2026-09-17 にそう測ったという記録として書き換えない**（§7.0 の規律）。**決定の中身・出所・そして「解かれていないもの」は §7.18 に書く。**⚠ **この決定が解いたのは ⚠3 だけである**——**⚠4（項目6 の排他性）は、この決定とは別に、[ADR 0203](./decisions/0203-memories-omitted-exclusivity.md)（採用 2026-09-17、PR #435）のコードの修理で塞がれている。**⚠ **2026-09-26 訂正**: 当初ここは「⚠4 はそのまま残っている」と書いていたが、この追記を書いた 2026-09-21 の時点で既に誤りだった（§7.18 の「解いていないこと」1番）。
...
#### ⛔ この決定が *解いていない* こと —— **4つ挙げた。いま残っているのは2〜4番の3つ**（⚠ 当初は「4つ残っている」と書いた。1番は 2026-09-26 に訂正した）

1. **⚠4（項目6 の排他性）は、この決定では解かれていない。**⚠3 と ⚠4 は同じ形（段3.5 を勘定に入れていない）だが、
   **オーナーが解いたのは ⚠3 だけである。**
   ⚠ **2026-09-26 訂正: ただし ⚠4 の穴は、この節を書く前に既に塞がれていた。**当初ここは「**⚠4 は残っている**」と書いていたが、
   2026-09-21 の時点で既に誤りだった——⚠4 が記録した「同じ `memoryId` が `memories` と `omitted` の両方に載る」は、
   [ADR 0203](./decisions/0203-memories-omitted-exclusivity.md)（採用 2026-09-17、[Issue #421](https://github.com/takecchi/mnemora/issues/421) / PR #435）が
   `below_threshold` から取り下げる形で塞いでおり、実装
   （`grep -Fn -- '排他性契約（Issue #421 / ADR 0203）' packages/core/src/recall-runtime.ts`）と
   テスト（`packages/core/src/__tests__/recall-association.test.ts` の「memories と omitted の排他性」）が `main` に在る。
   ⛔ **⚠4 を閉じたのはオーナーの決定ではなく、コードの修理である**（§7.17 が #402 について引いたのと同じ区別）。
   ⚠ **ADR 0203 が自ら引き受けた負債（取り下げるのは `below_threshold` だけで、件数しか持たない他の kind は対象外）は残っている。**
...
3. **既定は off のままである。**⟹ **出荷される `@mnemora/core` の既定の振る舞いは、この決定では1バイトも動いていない。**
   ...
   **⚠4 が記録した「同じ `memoryId` が `memories` と `omitted` の両方に載る」は、ADR 0203 の修理より前に測った事実として残っている**
   （⚠ 2026-09-26 訂正: 当初は「測った事実として残っている」とだけ書いており、穴がいまも開いているように読めた。穴は上の1番のとおり塞がれている）。

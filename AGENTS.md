...
| `recorded` | 記録した実 API の応答の再生。**記録に無い入力は例外** | 北極星の物差し（`retrieval` / `compare`）。Issue #109 後半以降は `identifier-probes` / `numeral-token-probes` の**追加 arm**（OpenAI 実埋め込み6群、専用カセット `examples/chat/cassettes/identifier-probes.openai.json` / `numeral-token-probes.openai.json`）も同じ層で再生する——⚠ **主測定（`local` を固定で使う）とは別の、追加の測定**であり、下の行の「4ジョブとも `local` を固定で使う」を上書きしない（[ADR 0315](./docs/decisions/0315-openai-embedding-false-positive-ceiling.md)） |
...
**⚠ 上の4ジョブ（`identifier-probes` / `numeral-token-probes` / `consolidation-cost` / `archive-sweep-cost`）の数字を
...
- **`Co-Authored-By:` のトレーラと `🤖 Generated with [Claude Code]` を付けない**
  ...
  - **規約の有無を、履歴の分布から推定しないこと。**決めているのはこの節である
    （推定して向きを逆に報告した経緯と、正しい数え方（`grep -i`）は
    [ADR 0113](./docs/decisions/0113-no-attribution-trailers.md) に在る）。

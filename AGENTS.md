...
| `recorded` | 記録した実 API の応答の再生。**記録に無い入力は例外** | 北極星の物差し（`retrieval` / `compare`）。Issue #109 後半以降は `identifier-probes` / `numeral-token-probes` の**追加 arm**（OpenAI 実埋め込み6群、専用カセット `examples/chat/cassettes/identifier-probes.openai.json` / `numeral-token-probes.openai.json`）も同じ層で再生する——⚠ **主測定（`local` を固定で使う）とは別の、追加の測定**であり、下の行の「4ジョブとも `local` を固定で使う」を上書きしない（[ADR 0310](./docs/decisions/0310-openai-embedding-false-positive-ceiling.md)） |

 ...
 * （ADR 0198「採らなかった案」）。⟹ **`@mnemora/anthropic` 側にも同じ形の歯を置いてある**
 * （`packages/anthropic/src/__tests__/call-failure.test.ts`）。**片方だけ直さないこと。**
 *
 * ⚠ **測っているのは「この wrapper がリトライしないこと」であって、「production の
 * 経路がリトライしないこと」ではない。** `client` を注入しているため、`new OpenAI()`
...
describe("OpenAILLMProvider — 呼び出し自体が失敗したとき（core の interface の契約）", () => {
  it("complete: SDK の呼び出しが reject したら、その例外をそのまま伝播し、リトライしない", async () => {
    ...
    // ⚠ `complete` は応答が空でも例外にせず空文字を返す形なので（ADR 0072 負債2）、
    //    **ここが黙って `{ content: "" }` に倒れないこと**が特に効く。
    await expect(

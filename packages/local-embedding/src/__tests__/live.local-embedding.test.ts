 ...
 * 繋がないので、API 料金は発生しない。要求するのは、**4ファイル計42MB（うち重み36MB）のダウンロードと、
 ...
 * ——⛔ 契約を緩めて通さないこと**（ADR 0099「これが覆るとしたら」の規律）。
 *
...
describe.skipIf(!live)(
  ...
  () => {
    describeEmbeddingProviderConformance({
      ...
      timeout: 300_000,
      // ⚠ `overLimitText`（Issue #449 / ADR 0304）は、ここでも渡さない。
      // 本物のトークナイザに当たる場所なので測る意味はあるが、ADR 0090 が釘を刺している
      // とおり「字数」は上限（トークン数）の代わりにならず（単純な繰り返し文字列は BPE で
      // 圧縮され、`"あ".repeat(100)` が15トークンにしかならない実測がある）、この歯を
      // 走らせずに正しい「確実に上限を超える」文字列を用意する自信が無い。⟹ 無理に足さず、
      // **既に同じファイル内にある「歯1〜3」（上記、本物のトークナイザで二分探索して
      // 求めた8192/8193トークン境界を使う）に測定を委ねる。**
    });
  },
);

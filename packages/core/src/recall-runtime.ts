...
export async function runRecall(
  ...
  // -------------------------------------------------------------------
  // 段3.5: 連想（既定 on。docs/recall.md §9、ADR 0151、既定は ADR 0187 で反転）
  //
  ...
  //
  // **既定 on／`null` で明示的に off（ADR 0187）**: `validatedQuery.association` が
  // `undefined`（省略）なら DEFAULT_RECALL_ASSOCIATION を使い、`null`（明示）なら
  // この段全体を丸ごとスキップする——北極星の問い2（無効にしても成立するか）の
  // 担保先が、ADR 0151 の「既定 off」から「`null` という明示の opt-out」へ移った。
  // -------------------------------------------------------------------
  ...
  // （ADR 0187）になったので、`associationQuery`（省略時は DEFAULT_RECALL_ASSOCIATION、

...
export interface Observation {
  ...
  /**
   * Issue #152（ADR 0307）: 呼び手が申告した任意属性を `occurredAt`/`validFrom` と同じ経路
...
export const ObservationSchema = z.object({
  ...
  // Issue #152（ADR 0307）: `Observation.attributes` の doc コメント参照。格納側は
...
export interface ObserveUtteranceInput {
  ...
  /** {@link Observation.attributes} の doc コメント参照（Issue #152、ADR 0307）。 */
  attributes?: Attributes;
  speaker?: string;
  text: string;
}

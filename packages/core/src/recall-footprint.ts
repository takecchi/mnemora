 ...
 *
 * ⚠⚠ **[ADR 0187](../../../docs/decisions/0187-recall-association-default-on.md) が
 * `recall()` 自身の連想枠の既定を on にした後も、この関数の `associationCount` の既定は
 * `0` のままである（据え置いた。値を `DEFAULT_RECALL_ASSOCIATION.maxCount` 等に
 * 合わせて変えていない）。** 理由——`maxCount` は連想枠が**試みる上限**であって、
 * **実際に何件が本体へ昇格するか**（ANN の近傍分布・`minSimilarity` の閾値に依存する、
 * 上のパラグラフの前提そのもの）とは無関係である。`maxCount` をそのまま
 * `associationCount` の既定に流用すると、「呼び出し側にしか無い値」を
 * `packages/core` が代わりに推測することになり、この節が最初に立てた原則
 * （ここで推定しない）を自ら破る。**⟹ `RecallQuery.association` を省略した
 * （＝連想枠が既定 on で走る）呼び出しについて、`associationCount` も同時に省略すると、
 * この関数の見積もりは実際の `recall()` の出力を体系的に過小評価する**
 * （北極星「目指す姿」6本目「知らないことを、知らないと言える」に照らすと、
 * この過小評価は「探していない」を「見つからなかった」と同じ顔で返す形に近い）。
 * 正確に見積もりたい呼び出し側は、`footprintSampleFromRecall` による較正か、
 * 過去の実測（例: `association-probes` ベンチ・ADR 0168。`maxCount=10` で
 * スコープ内から実際に昇格する件数は 12件の probe で 0〜10件、単調ではない）から
 * 見積もった値を明示的に渡すこと。**詳細と、この判断の理由・危険は ADR 0187 を参照。**
 */
...
export interface RecallFootprintShape {
   ...
   *
   * ⚠⚠ **[ADR 0187](../../../docs/decisions/0187-recall-association-default-on.md) 以降、
   * 「省略時は `0`」は「連想枠を一切使わない呼び出しと1バイトも変わらない」ことを
   * **もう意味しない**——`recall()` 自身の連想枠の既定が on になったため、
   * `RecallQuery.association` を省略した通常の呼び出しでも連想は走る。**この既定
   * （`0`）は意図して据え置いた**（ADR 0166 以前の値のまま、`DEFAULT_RECALL_ASSOCIATION`
   * には連動させていない）。⟹ `associationCount` を渡さずに見積もると、
   * `RecallQuery.association: null` で明示的に止めた場合を除き、**見積もりは実際より
   * 小さく出る**（構造上の上限で切り詰められるため過大評価にはならない。過小評価に
   * のみ倒れる）。据え置いた理由と、この過小評価の危険は ADR 0187 に書いてある。
   *

 ...
 * Phase 2 向けに残す）。
 *
 * **⚠ 2026-09 追記（Issue #152/#153、ADR 0310）: `attributes`（`RecallQuery.attributes`）は
 * `tenant`/`subject` と同じ側——スコープの外側の境界である。** `attributes` で絞り込んだ
 * 結果は `filtered` として報告しない（`FilteredOmission.condition` に専用の値を足さない
 * ——ADR 0310「採らなかった案」参照）。`totalInScope` はこの絞り込みの内側だけを数える。
...
export interface RecalledMemory {
   ...
   * なら「絞り込みに使える軸は載せる。使えない詳細は `get()` に残す」**（ADR 0310）。
   *
...
export interface RecallQuery {
  ...
  tags?: string[];
  /**
   * **母集合を段1（候補生成）で減らす、AND 等値の絞り込み**（Issue #152/#153、ADR 0310）。
   *
   ...
   * 2つを同じ絞り込み意味論に混ぜない。
   *
   * **意味論は AND 等値だけ**（ADR 0310 決定5）。渡したキーすべてが、その Memory の
   ...
   * への値追加が破壊的変更に当たるかどうかは #541 が未決であり（ADR 0310「採らなかった
...
export interface RecallScope {
  ...
  includeSubjectless?: boolean;
  /**
   * Issue #152/#153（ADR 0310）: `RecallQuery.attributes` がそのまま入る（空オブジェクト

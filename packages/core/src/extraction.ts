...
export interface BuildNewMemoryParams {
  ...
  /**
   * [ADR 0163](../../docs/decisions/0163-decay-activity-clock.md) 決めたこと3・5:
   ...
   * `undefined` のまま Memory に書かれる（ADR 0163 決めたこと5「`'wall'` のテナントでは
   * 何も増えない」）。
   */
...
export function buildNewMemoryFromCandidate(params: BuildNewMemoryParams): NewMemory {
  ...
  // ADR 0163 決めたこと3・5: 活動時計の3つ組。`activitySeq`/`halfLifeRecalls` の両方が

...
export interface VectorFilter {
  ...
  occurredBefore?: Date;
  /**
   * 活動時計の忘却ゲート（[ADR 0157](../../../../docs/decisions/0157-decay-activity-clock.md)
   ...
   * 活動時計では沈まない」（ADR 0157 決めたこと4）——`decayFloorSeqAfter` を渡しても、
   * `decay_floor_seq` が無い行を落としてはならない。
   */
  decayFloorSeqAfter?: number;
  /**
   * `decayFloorAtAfter` と `decayFloorSeqAfter` の結び方を切り替える（ADR 0157 決めたこと1、

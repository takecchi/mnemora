...
/**
 * Issue #371（(B) 第1段、ADR 0185 決定2・ADR 0312）: 「この記憶は何についての主張か」を
 * 表す構造化された鍵。
 *
...
export const ClaimKeySchema = z.object({
  /** 主張の主語（例: `"user"`）。ADR 0312 §2 の実験に倣い、正規化済み文字列を想定する。 */
  subject: z.string().min(1),
  /** 主張の述語（属性名）。正規化済みの英語 snake_case を想定する（ADR 0312 決定3）。 */
  predicate: z.string().min(1),
});
export type ClaimKey = z.infer<typeof ClaimKeySchema>;

/**
 * ADR 0312 決定3 の正規化規則: **NFKC 正規化 → 前後の空白除去 → 小文字化 →
 * 内部の連続空白を単一の `_` に畳む。**
 *
 ...
 * 行わない。** その統合は LLM 自身が行う（ADR 0312 §3.2 の実測: バッチ内で常に100%
 ...
 * **完全に独立した別の system 文面**（ADR 0312 決定1・決定2）。この文字列を変えても
...
/**
 * ADR 0312 決定2 の (ii) separate: 既存の抽出（`extraction.ts` の `extractCandidates`）が
 ...
 *
 * `contents.length === 0`（候補が0件）なら**呼び出しを一切行わない**（ADR 0312 決定2
 * 「候補が0件なら+0回にできる」）。
 */

...
export interface ScoringInput {
  ...
  /**
   * [ADR 0163](../../../../docs/decisions/0163-decay-activity-clock.md) 決めたこと12:
   ...
   * （NULL）＝活動時計では沈まない」（ADR 0163 決めたこと4）と同じ向きの判断である。
   */
...
/**
 * 段2の再スコア係数 `decay`（ADR 0163 決めたこと12）。
 *
 ...
 *   ADR 0163 決めたこと4「NULL はこの軸に床が無い＝活動時計では沈まない」と同じ向きの
 *   判断であり、活動時計だけを使おうとして値が無い場合に `decay = 0`/`NaN` へ倒すのは
 *   その向きに反する。
 * - `'either'`: **2つの係数の `Math.max`**（最も緩い）を使う。ADR 0163 決めたこと1が
...
function computeDecay(input: ScoringInput): number {
  ...
  if (!hasActivityInputs) {
    // 揃っていない＝この軸に床が無い（ADR 0163 決めたこと4）。壁時計へフォールバックする。
    return wallDecay;
  }

...
export interface ScoringInput {
  ...
  /**
   * **段2の時間項の方針**（Issue #690、
   * [ADR 0295](../../../../docs/decisions/0295-time-weighting-policy-opt-in.md)）。
   *
 ...
 *   変えない**（ADR 0295 §2 のケース B・C が陽性対照として `total` の完全一致を
 *   固定している）。
 *
 * **⛔ どちらの値でも `decay` は変えない。**`decay`（使用の新しさ、`reinforce` で
 * 若返る）と `freshness`（内容の新しさ、出来事時刻の古さ）は独立な軸のままである
 * ——分離するはずの2軸を、別の形でまた結合しない（ADR 0295 §4.2 が「起点を
 * 共有させる」案を却下した理由そのもの）。
 *
 ...
 * （ADR 0295 §7 が「オーナー判断」として開いたまま残している点）。
 */
...
/**
 * `freshness` の計算そのもの（ADR 0036 の頭打ちに加え、Issue #690 / ADR 0295 の
 ...
 *   ADR 0295 §4.2 案(1)）。`occurredAt` が在るときは `"legacy"` と同じ式を通る。
 */

...
/**
 * `attributes` の積集合（ADR 0306 決定4）: `eligible` **全件**に同じキー・同じ値で
 ...
 * 引き継ぎ方もこの2つの経路で意図的に揃える——本 PR（Issue #152/#153、ADR 0306）の
 * 判断。理由: `attributes` は内容ではなく取り扱い（公開範囲など）を表す軸であり、
 * 統合・反芻のどちらも「元の記憶の集合から新しい記憶を作る」という点で対称だから
 * である（迷ったときは積集合＝落とす方向へ倒す、ADR 0306 決定4）。
 *
 ...
 * - `attributes`: eligible 全件の積集合（`intersectAttributes`、ADR 0306 決定4）。
...
export function buildConsolidatedMemory(params: BuildConsolidatedMemoryParams): NewMemory {
  ...
  return {
    ...
    tags,
    // Issue #153（ADR 0306 決定4）: 積集合。`intersectAttributes`（上）参照。

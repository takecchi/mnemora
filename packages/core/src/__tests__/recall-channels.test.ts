...
describe("recall() — 歯②: 既定(channels 未指定)は ADR 0084 以前と1バイトも変わらない", () => {
      ...
      explain: {
        stages: [
          ...
          },
          {
            ...
            // 箇所そのもの。PR 本文参照)。
            // clock: ADR 0163 決めたこと1・12（北極星の問い3）で足した欄。テナント設定行が
  ...
  it("②-c 形の一致: candidate_generation の trace はちょうど1つ、score に lexicalMatch という鍵が無い", async () => {
    ...
    // detail が過不足なく { channel, kPrime, hits, decayGate, clock } であること
    // （toEqual は多すぎず少なすぎずを見る。decayGate は ADR 0153、clock は ADR 0163 で
    // 増えた欄）。

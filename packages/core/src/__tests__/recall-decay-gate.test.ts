...
describe("recall() — 忘却ゲートの時計選択（ADR 0158 決めたこと1）", () => {
  ...
  it("'activity' のテナントで decayFloorSeq が NULL（この軸に床が無い）なら常に生き残る（ADR 0158 決めたこと4）", async () => {
    const { runtime, stores } = buildRuntime();
    await stores.tenantSettingsStore.setDecayClock(ctx, "activity");
    const alive = await createEmbeddedMemory(stores, [1, 0], {
...
describe("recall() — 語彙チャンネルの後置フィルタにも活動時計が掛かる（ADR 0158 決めたこと12、忘れやすい非対称）", () => {
...
describe("recall() — 非破壊性: RecallRuntimeDeps.tenantSettingsStore を省略しても 'wall' として動く（ADR 0158 決めたこと13）", () => {
 ...
 *
 * `advanceActivityClock`（`NewRecallRecord`、ADR 0158 決めたこと5）は `decay_clock` が
...
describe("recall() — ⭐ 'activity' 単独で、素通りされ続けた記憶は壁時計より速く沈む（多忙なテナント）", () => {
  ...
  it("対照条件: 同じ4回の recall を 'wall' のテナントに対して行うと、壁時計だけを見るので何回呼んでも沈まない", async () => {
    ...
    // (ADR 0158 決めたこと5「advanceActivityClock は decay_clock != 'wall' のテナントに限る」)。

 ...
 * ADR 0307）は、段1（ANN・語彙の候補生成）と段3.5（連想枠）の後置フィルタ
...
describe("recall() — 必須の同伴取得（段3）にも attributes が掛かる（レビュー指摘、ADR 0307 追記）", () => {
...
describe("recall() — 目次帯（段5）にも attributes が掛かる（レビュー指摘、ADR 0307 追記）", () => {

...
/**
 * `RecallQuery.attributes`（Issue #152/#153、ADR 0310）の歯。
 *
 ...
 *    （`subjectId`/`tenant` と同じ「スコープの外側の境界」——ADR 0310 決定6）。
...
describe("recall() — attributes が実際に候補を落とす（AND 等値、Issue #152/#153）", () => {
  ...
  it("attributes で落ちた Memory は omitted に出ない（subjectId/tenant と同じスコープの外側、ADR 0310 決定6）", async () => {
    const { runtime, stores } = buildRuntime();

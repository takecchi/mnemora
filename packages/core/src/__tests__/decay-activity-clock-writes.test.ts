...
/**
 * [ADR 0158](../../../docs/decisions/0158-decay-activity-clock.md) 決めたこと3・5・12 の
...
describe("runtime.observe（抽出） — 活動時計の3つ組の配線（ADR 0158 決めたこと3・5・12）", () => {
  it("'wall' のテナント（既定）では decayBaseSeq/decayFloorSeq/halfLifeRecalls が3つとも undefined のまま——tenant_activity を読まない", async () => {
    ...
    // ADR 0158 決めたこと2 の doc「tenant_settings は読み出しの多い設定行」——
  ...
  it("'activity' のテナントでは decayBaseSeq=activity_seq・halfLifeRecalls=既定値・decayFloorSeq が計算されて書かれる", async () => {
    ...
    // createRecall の同じ経路を直接使う(ADR 0158 決めたこと5 と同じ書き込み口)。
    for (let i = 0; i < 3; i += 1) {
...
describe("runtime.consolidate — 活動時計の3つ組の配線（同じ resolveActivityClockInputs を通る、ADR 0158 決めたこと3・5・12）", () => {
...
describe("runtime.reflect — 活動時計の3つ組の配線（同じ resolveActivityClockInputs を通る、ADR 0158 決めたこと3・5・12）", () => {

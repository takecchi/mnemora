...
class FakeBackingStore {
  ...
  events: MemoryEvent[] = [];
  /**
   * [ADR 0158](../../../docs/decisions/0158-decay-activity-clock.md) 決めたこと2・5:
...
export class FakeMemoryStore implements MemoryStore {
  ...
  private createMemoryIdempotent(ctx: Ctx, input: NewMemory): IdempotentCreateResult<Memory> {
    ...
    return resolveIdempotentCreate(existing, () => {
      ...
      const memory: Memory = {
        ...
        // ADR 0158 決めたこと3: 活動時計の3つ組。以前はここで1つも転記しておらず、
  ...
  async createRecall(ctx: Ctx, record: NewRecallRecord): Promise<RecallId> {
    ...
    // ADR 0158 決めたこと5: `recalls` への INSERT と「同一トランザクション」で
...
export class FakeVectorStore implements VectorStore {
  ...
  async search(
    ...
    for (const [key, entry] of this.entries) {
      ...
      // ADR 0158 決めたこと1・12・14: 忘却ゲートの2軸。`decayFloorAnyAxis: true` かつ
...
// 独立に検査する歯は無い。
//
// [ADR 0158](../../../docs/decisions/0158-decay-activity-clock.md) 決めたこと13:
...
export class FakeTenantSettingsStore implements TenantSettingsStore {
  ...
  constructor(
    ...
    /**
     * ADR 0158 決めたこと13 末尾の訂正どおり、`activity_seq` を進めるのは
   ...
   * （ADR 0158 決めたこと13 が足したのは読むだけの `getDefaultHalfLifeRecalls`）。

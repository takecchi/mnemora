...
export class FakeMemoryStore implements MemoryStore {
  ...
  private createMemoryIdempotent(ctx: Ctx, input: NewMemory): IdempotentCreateResult<Memory> {
    ...
    return resolveIdempotentCreate(existing, () => {
      ...
      // Issue #201 PR-B（ADR 0320）: `packages/testkit` の `InMemoryMemoryStore` と同じ
  ...
  /**
   * Issue #201 PR-B（ADR 0320）: `listLabels?`（`InMemoryMemoryStore.listLabels` と同じ契約）。
   */
  ...
  /**
   * Issue #201 PR-B（ADR 0320）: `registerLabel?`（`InMemoryMemoryStore.registerLabel` と
   * 同じ契約）。
   */
  ...
  async aggregateScope(
    ...
    for (const memory of this.backing.memories.values()) {
      ...
      // Issue #201 PR-B（ADR 0320）: taxonomy ゲート。`attributes`（上）とは違い
    ...
    // Issue #201 PR-B（ADR 0320「決定5」）: `packages/testkit` の `InMemoryMemoryStore` と
    // 同じ意味論。
...
export class FakeTenantSettingsStore implements TenantSettingsStore {
  ...
  /**
   * Issue #201 PR-B（ADR 0320）: `getTaxonomyMode?`（`InMemoryTenantSettingsStore` と
  ...
  /**
   * Issue #201 PR-B（ADR 0320）: `setTaxonomyMode?`（`InMemoryTenantSettingsStore` と
   * 同じ契約）。
   */

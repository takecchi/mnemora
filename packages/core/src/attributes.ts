...
/**
 * 呼び手が申告する任意属性（Issue #152 / #153、ADR 0307）。
 *
 ...
 * 由来の違う2つの値を同じ欄に混ぜない——ADR 0307 の決定1・「採らなかった案」参照。
 *
...
/**
 * `Attributes` の上限（ADR 0307 決定7「キー空間の規律」）。
 *
 ...
 * **緩めるのは後からできるが、締めるのは破壊的変更になる**（ADR 0307 決定7・
...
/**
 * キーの文字種（ADR 0307 決定7）。ASCII の英数字・`_`・`-`・`.`・`:` に絞る。
 *
 ...
 * 等、格納・伝播側の型には validation を持たせない（ADR 0307 決定2「欄を渡したときだけ
 * 検査される」)。
 */

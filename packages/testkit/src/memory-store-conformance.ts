...
export function describeMemoryStoreConformance(options: MemoryStoreConformanceOptions): void {
  ...
  describe(`MemoryStore conformance (${name})`, () => {
    ...
    // -------------------------------------------------------------------
    // attributes（Issue #152/#153、ADR 0307）
    // -------------------------------------------------------------------
    ...
    it("aggregateScope は scope.attributes で絞り込める。落ちた分は totalInScope から静かに除かれる（omitted 相当の報告は無い、ADR 0307 決定6）", async () => {
      const store = await createStore();
      const ctx: Ctx = { tenantId: "tenant-1" };
    ...
    it("aggregateScope の digests（目次帯の候補）も scope.attributes で絞られる（レビュー指摘、ADR 0307 追記）", async () => {
      const store = await createStore();
      const ctx: Ctx = { tenantId: "tenant-1" };

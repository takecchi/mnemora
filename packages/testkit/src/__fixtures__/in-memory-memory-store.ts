...
export class InMemoryMemoryStore implements MemoryStore {
  ...
  /**
   * Issue #201 / ADR 0306: `labels` 相当のインメモリ表。key は `${tenantId}::${name}`。
  ...
  /**
   * Issue #201 / ADR 0306: `PostgresMemoryStore.upsertProposedLabels` と同じ契約——
  ...
  private createMemoryIdempotent(
    ...
    return resolveIdempotentCreate(existing, () => {
      ...
      // Issue #201 / ADR 0306: `createMemory`/`createMemoryWithOutbox`/

 ...
 * クローン miku の判断）。
 *
 * `packages/postgres` の `LEXICAL_QUERY_MAX_DISTINCT_WORDS`
 ...
 *
 * **⚠ `tokenize()` は非文字・非数字の連なり（ハイフン・ピリオド等を含む）をすでに
 * 区切り文字として分割する**（`\p{L}\p{N}` の否定クラス）ため、空白を含まない記号
 * つなぎの長い「1語」は、この時点ですでに複数の語に分かれ、`Set` で重複も畳まれる
 * ——postgres 側で語ごとの文字数の上限が要った理由（`websearch_to_tsquery` が語の中の
 * 記号を区切りと解釈するのに、区切り**前**の空白分割ではそれが見えない）は、
 * ここにはそもそも無い。**それでも語ごとの文字数の上限を同じ形で入れる**のは、
 * 3実装の挙動をできるだけ揃えるため（記号を含まない、単に長いだけの1語を落とす
 * 場合の挙動が実装間でずれないようにする）。
 */
...
export class InMemoryLexicalStore implements LexicalStore {
  ...
  async search(
    ...
    // Issue #878: 異なる語数に上限を置く（capQueryTerms の doc 参照）。
    const queryTerms = capQueryTerms(tokenize(query));

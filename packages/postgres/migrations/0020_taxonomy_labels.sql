...
--
-- Issue #201 / ADR 0315（案(1)）: taxonomy の語彙管理（labels / memory_labels）を
-- Phase 2 の予定から、任意の追加として前倒しで実装する PR-A。
-- docs/memory-model.md §8 の SQL 案をそのまま採用する。
--
-- 決定の詳細は ADR 0315 を見ること。ここでは移行そのものに関係する要点だけ書く。
--
...
-- インクリメントする（ADR 0315）——ここで起動時点の値を揃えておく。
--
...
-- （1 Memory ＝ 1回のインクリメント、`tags` 内の重複は `Set` で潰す。ADR 0315 参照）

...
export class PostgresMemoryStore implements MemoryStore {
  ...
  async aggregateScope(
    ...
    // Issue #201 PR-B（ADR 0320「決定5」）: `RecallQuery.taxonomyGroups: true` のときだけ
    ...
    const result = await this.db.execute(sql`
      ...
      agg AS (
        SELECT
          ...
          -- Issue #201 PR-B（ADR 0320）: taxonomy ゲートで落ちた件数。period_filtered/
    ...
    // Issue #201 PR-B（ADR 0320「決定5」）: `taxonomyGroupCandidates` が渡されたときだけ

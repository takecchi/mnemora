 ...
 * と同じ分担(Issue #340 フォローアップ、ADR 0307)。
 *
 ...
 * いない——ADR 0307 §2 の決定どおり、`examples/chat/compare-baseline.json` のような
 * ⭐門の CI-sourcing 手順(ADR 0119/0121/0133、artifact を2回以上取り、一致した値だけを
 * 基準値にする)を、この bench ではまだ踏めていない(手元の作業環境から CI artifact を
 * 取得する経路が無い——ADR 0307「引き受けた負債2」)。⟹ 他5本(retrieval-quality等)と
...
export function buildSummaryMarkdown({ measured, baseline }) {
  const lines = [];
  lines.push("## recall-footprint 較正の補助標本(Issue #340 フォローアップ、ADR 0307)");
  lines.push("");
  lines.push(
  ...
  if (baseline === undefined) {
    ...
    lines.push(
      "⚠ `--baseline` が渡されていない——CI artifact での2回以上一致をまだ実測して" +
        "いないため、この bench にはまだ基準値ファイルが無い(ADR 0307 §2)。",
    );
    return lines.join("\n");
  }

 ...
 * 🔴 **このファイルが固定している線**:
 *
 * 1. **server_encoding/nonAsciiIsIndexed がどちらでも exit 0**(⛔ 門ではない)。
...
describe("lexical-regime-summary.mjs(子プロセスで起動)", () => {
  ...
  it("🔴 nonAsciiIsIndexed が true でも false でも exit 0(⛔ 門ではないことの固定点)", () => {
    ...
    expect(run(["--measured", truePath]).status).toBe(0);

    const falsePath = writeJson(
      "false.json",
      makeValid({ nonAsciiIsIndexed: false, regime: "non_ascii_dropped", rawIdentifierHit: true }),
    );
    expect(run(["--measured", falsePath]).status).toBe(0);
  });

  it("🔴 serverEncoding が SQL_ASCII でも exit 0(門にしていないことの固定点)", () => {
  ...
  it("🔴 --measured のパスが存在しないと非0で、『値が出ていない』側の文言が出る", () => {
    const result = run(["--measured", join(tmpdir(), "does-not-exist-lexical-regime.json")]);
    expect(result.status).not.toBe(0);
  ...
  it("⭐ 『ファイルが無い』と『値が空だった』は異なるメッセージである", () => {
    const missing = run(["--measured", join(tmpdir(), "does-not-exist-lexical-regime-2.json")]);
    const emptyPath = writeJson("empty2.json", makeValid({ serverVersion: "" }));
    const empty = run(["--measured", emptyPath]);
  ...
  it("⭐ 『ファイルが無い』と『JSON が壊れている』も異なるメッセージである", () => {
    const missing = run(["--measured", join(tmpdir(), "does-not-exist-lexical-regime-3.json")]);
    const brokenPath = writeJson("broken2.json", "not json at all");
    const broken = run(["--measured", brokenPath]);

 ...
 * 射程外)・`4.42MB` / `4．42MB`(小数の一部に `42` が現れる別の値。2026-09-24、
 * Issue #455 で後ろ読みに半角・全角のピリオドを足した——それまでは後ろ読みが
 * 数字しか止めず、この形の中の `42MB` を拾っていた)。
 */
const SIZE_RE =
...
describe("SIZE_RE の射程(表記揺れ) — ADR 0212 追記(2026-09-21)", () => {
  ...
  it("HIT する(拾えるべき)表記をすべて拾う(既存5形+今回広げた5形)", () => {
    const cases = [
      ...
      ["42ＭＢ", "42"], // 全角MB
      ["42ＭｉＢ", "42"], // 全角MiB
      // 陽性対照(Issue #455、2026-09-24): 前に文字が在る正しい記載。後ろ読みを
      // 広げて小数点を止めても、空白・括弧・漢字の直後の値は拾い続けること
      ["重みは 42MB", "42"],
      ["（42MB）", "42"],
      ["約42MB", "42"],
    ];
  ...
  it("miss しなければならない(誤って拾ってはいけない)表記を拾わない", () => {
    const cases = [
      ...
      "41.9MB", // 36/42 とは別の値
      // Issue #455(2026-09-24): 小数の一部に 42 が現れる別の値。後ろ読みが数字しか
      // 止めていなかったので、以前は "4.42MB" の中の "42MB" を拾っていた
      "4.42MB",
      "4．42MB", // 全角ピリオド
      "約4.42MB",
    ];

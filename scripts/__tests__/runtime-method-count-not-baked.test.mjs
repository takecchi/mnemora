 ...
 * [ADR 0272](../../docs/decisions/0272-runtime-method-count-notation-sweep.md) が、
 ...
 * （算用数字/漢数字、助数詞の種類）の軸だけを広げる。**ADR 0272 に、広げた各パターンを
 ...
 *   この歯は主語がメソッド件数だと分かる形に錨を打ったままにしている
 *   （AGENTS.md「⚠ 偽陽性率に上限を置けない検査は門にしない」）。
 *   ⚠ **[Issue #606](https://github.com/takecchi/mnemora/issues/606) が「引き受けた」と
 *   読まないこと。**同 Issue の一般形の掃引は **行わないと決まった**——錨を打つと広域21
 *   ファイルで当たるのが1件だけで、それも別語義の偽陽性だったためである（判断の逐語は
 *   Issue #606 のコメント、2026-09-23。⚠ **クローンの判断であってオーナー本人の判定では
 *   ない**、ADR 0220）。⟹ **この範囲を拾う担い手は、いまは居ない。**
 * - `Runtime` 以外の interface（`MemoryStore` 等）のメソッド件数の焼き込みは見ていない
 *   （ADR 0269 の対象外、ADR 0270 / 0272 も引き継がない）。
...
//    変わらない側）の分布が多いため、錨を外すと偽陽性の山になる（ADR 0272、実測）。
//    数値部＋助数詞＋「の」＋（メソッド|口）という組み合わせは、対象4文書を横断 grep
//    した実測でどこにも当たらない（ADR 0272）。
